...
if ($erreur == 0)
{
	if($methode == "visu")
	{
		$req_stock = "select gobj_seuil_dex, gobj_seuil_force, gobj_tobj_cod, tobj_libelle, gobj_poids, gobj_pa_normal,
	...
	if($methode == "visu2")
	{
		$req_stock = "select obj_seuil_dex, obj_seuil_force, gobj_tobj_cod, tobj_libelle, obj_poids, gobj_pa_normal, gobj_pa_eclair,
	...
	$select_pane = 0;
	if(isset($_POST['pane'])) $select_pane = $pane;
	...
	if(isset($_POST['methode']))
	{
		switch ($methode)
		{
			case "create_mag_tran":
				...
				$erreur = 0;
				$req_verif = "SELECT gobj_valeur * $tran_quantite as prix_mini, mgstock_nombre as qte_dispo
				...
				if ($erreur == 0)
				{
					$req = "insert into transaction_echoppe
			...
			case "create_tran":
				...
				if(isset($_POST['article_cod']) && $article_cod) {
					// ARTICLE NORMAL
					$req_verif =  "select gobj_valeur*$tran_quantite as prix_mini,mgstock_nombre  as qte_dispo from objet_generique,stock_magasin_generique
						where gobj_cod = mgstock_gobj_cod and mgstock_lieu_cod = $lieu_cod
							and gobj_cod = $article_cod";
					$db->query($req_verif);
					if(!$db->next_record()){
					...
					// ARTICLE SPECIAL
					$req_verif = "select obj_cod,obj_nom,gobj_cod,obj_valeur
				...
				if(!$db->next_record()){
					...
					if($erreur == 0){
						...
						if($db->next_record()){
							...
							$req_tran =  "insert into stock_magasin_generique (mgstock_nombre,mgstock_lieu_cod,mgstock_gobj_cod)
...
<p class="titre">Gestion de : <?php  echo $lieu_nom?> - (<?php  echo $pos_x?>, <?php  echo $pos_y ?>, <?php  echo $etage_libelle?>, <?php  echo $lieu_compte?> Br en caisse)</p>
<div align="center">
<div id="intro" class="tableau2">
	<p>Information aux gérants	</p>
	<p>Ceci est une page en cours de développement, ce qui veut principalement dire qu'elle ne fonctionne pas. Elle va changer, contenir des essais et des idées qui viendront plus ou moins vite (ou pas du tout). Mais vous pouvez faire des essais et des remarques si vous voulez. Pour cela contactez Mirreck ou votre admin commercial préféré.</p>
  ...
  <p><b>31/01/2008</b> Ajout de la fonctionnalité pour changer la description et le nom du magasin</p>
	<p><b>28/12/2009</b> De nombreuses modifications / Corrections ont été apportées. L'une va vous toucher directement, avec la limitation des approvisionnements en fonction de l'échoppe. Certains articles ne pourront plus être approvisionnés librement. Adressez vous à votre Maitre Marchand préféré ...</p>
	Clients présents dans l’échoppe:
<?php 
	$liste_clients = "";
	$req_vue = "select lower(perso_cod) as minusc, perso_cod, perso_nom
...
?>
		...
		<p>Votre magasin est un refuge. Si vous souhaitez abandonner cette fonctionnalité, les prélèvements de l'administration passeront automatiquement à 15%.<br>
		<p style=text-align:left><b><a href="javascript:document.refuge.submit();">Abandonner le statut de refuge pour cette échoppe ? <i>(Cette fonctionnalité sera dorénavant controlée)</i></a></b>
<?php 
	}
?>
	</form>
  <form name="marge" method="post" action="gere_echoppe4.php">
	...
	<input type="hidden" name="methode" value="marge">
				...
				<p style=text-align:left><b><a href="javascript:document.marge.submit();">Changer cette donnée ?</a></b>
	</form>
...
<?php 
$req_stock = "select gobj_cod,gobj_nom,gobj_valeur,gobj_echoppe_stock,gobj_echoppe_destock,mgstock_nombre,mgstock_vente_persos,mgstock_vente_echoppes,comp_libelle,mgaut_gobj_cod from objet_generique
...
?>
	...
	Article :<select name="article_cod" onChange="document.begintransaction.article_special_cod.selectedIndex = 0;">
...
?>
	...
	Article Special:<select name="article_special_cod" onChange="document.begintransaction.article_cod.selectedIndex = 0;">
	<option value="">Choisir un objet</option>
<?php 
$req_stock = "select obj_cod,obj_nom,obj_valeur,gobj_cod
from objets,objet_generique,stock_magasin
where mstock_lieu_cod = $lieu_cod
				and mstock_obj_cod = obj_cod
				and obj_gobj_cod = gobj_cod
...
$db->query($req_stock);
  ...
  <table width="100%">
  <tr><td>Client</td><td>Objet</td><td>Quantité</td><td>Prix</td><td>Annuler</td></tr>
  <?php 
  $req_stock = "select perso_nom,tran_cod,obj_nom,tran_quantite,tran_prix
...
$db->query($req_stock);
			while($db->next_record())
			{ ?>
		<tr><td><?php echo $db->f("perso_nom")?></td><td><?php echo $db->f("obj_nom")?></td><td><?php echo $db->f("tran_quantite")?></td><td><?php echo $db->f("tran_prix")?></td>
    <td><a href="javascript:document.cancel_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.cancel_tran.submit();">Annuler</a></td></tr>
			<?php 
			}
?>
 <?php 
  $req_stock = "select perso_nom,tran_cod,gobj_nom,tran_quantite,tran_prix
...
$db->query($req_stock);
			while($db->next_record())
			{ ?>
		<tr><td><?php echo $db->f("perso_nom")?></td><td><?php echo $db->f("gobj_nom")?></td><td><?php echo $db->f("tran_quantite")?></td><td><?php echo $db->f("tran_prix")?></td>
    <td><a href="javascript:document.cancel_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.cancel_tran.submit();">Annuler</a></td></tr>
			<?php 
			}
?>
...
?>
	...
	Quantité :<input type="text" name="tran_quantite" value="1"><br><br>
	Prix total Proposé :<input type="text" name="tran_prix" value="0"><br><br>
	<font color="red"> <b>Attention: à ce prix sera ajouté 10% sur le montant total pour le transport !</b></font>
	<input type="submit" value="Valider la transaction !">
	</form>
  ...
  <tr><td>Client</td><td>Objet</td><td>Quantité</td><td>Prix</td><td>Annuler</td></tr>
  <?php 
  $req_stock = "select lieu_nom,tran_cod,gobj_nom,tran_quantite,tran_prix
...
$db->query($req_stock);
			while($db->next_record())
			{ ?>
		<tr><td><?php echo $db->f("lieu_nom")?></td><td><?php echo $db->f("gobj_nom")?></td><td><?php echo $db->f("tran_quantite")?></td><td><?php echo $db->f("tran_prix")?></td>
    <td><a href="javascript:document.cancel_mag_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.cancel_mag_tran.submit();">Annuler</a></td></tr>
			<?php 
			}
?>
  ...
  Les transactions qu'on me propose:
    ...
    <input type="hidden" name="transaction_cod" value="">
	  ...
	  <table width="100%">
  <tr><td>Vendeur</td><td>Objet</td><td>Quantité</td><td>Prix de base</td><td>Frais de transport</td><td>Prix total</td><td>Action</td></tr>
  <?php 
  $req_stock = "select lieu_nom,tran_cod,gobj_nom,tran_quantite,tran_prix
  from transaction_echoppe,lieu,objet_generique
where
...
$db->query($req_stock);
			while($db->next_record())
			{ ?>
		<tr><td><?php echo $db->f("lieu_nom")?></td><td><?php echo $db->f("gobj_nom")?></td><td><?php echo $db->f("tran_quantite")?></td>
    <td><?php echo $db->f("tran_prix")?></td><td><?php echo 0.1*$db->f("tran_prix")?></td><td><?php echo 1.1*$db->f("tran_prix")?></td>
    <td><a href="javascript:document.accepter_mag_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.accepter_mag_tran.submit();">Accepter</a></td>
    <td><a href="javascript:document.refuser_mag_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.refuser_mag_tran.submit();">Refuser</a></td></tr>
			<?php 
			}
?>
...
?>
<a href="javascript:document.refresh_comptes.tran_mois.value=<?php echo $t_mois-1?>;document.refresh_comptes.submit();"> &lt;&lt; Mois précédent</a>
<?php 
if($t_mois < 0){
?>
<a href="javascript:document.refresh_comptes.tran_mois.value=<?php echo $t_mois+1?>;document.refresh_comptes.submit();">Mois suivant &gt;&gt;</a>
<?php  } ?>
...
<table width="100%">
<tr><td class="soustitre2">Date</td>
...
$db->query($req);
			while($db->next_record())
			{  ?>
		<tr><td><?php echo $db->f("date_tran")?></td><td><?php echo $db->f("perso_nom")?></td><td><?php echo $label_type[$db->f("mgtra_sens")]?></td><td><?php echo $db->f("gobj_nom")?></td> <td><?php echo $db->f("mgtra_nombre")?> </td> <td><?php echo $db->f("mgtra_montant")?> </td>    </tr>
    <?php }
?>
</table>
<?php 
...
?>
<div class="tableau">
<b><?php echo $db->f("frm_nom")?></b> Temps nécéssaire:     <b><?php echo $db->f("frm_temps_travail")?></b> h <br/>
Composants:      <b><?php echo $db->f("frm_cout")?></b> Br<br/>
<?php 
	$req_comp = "select gobj_nom,frmco_num,mgstock_nombre from objet_generique,formule_composant 
