...
$req_quete        =
    "select pquete_quete_cod,pquete_perso_cod,pquete_nombre,pquete_date_debut,pquete_termine from quete_perso where pquete_perso_cod = $perso_cod and pquete_quete_cod = 18";
    ...
    switch ($methode3)
    {
        ...
        case "N":
            ...
            if ($stmt->rowCount() != 0)
            {
                ...
                $stmt      = $pdo->query($req2);
                $req2      =
                    "update quete_perso set pquete_termine = 'O' where pquete_quete_cod = 18 and pquete_perso_cod = " . $perso_cod;
                $stmt      = $pdo->query($req2);
                ...
                $stmt      = $pdo->query($req);
                $req       =
                    "select insere_evenement($perso_cod, $perso_dame_cygne, 100, '[perso_cod1] a ramené une fleur à la Dame Cygne.', 'O', 'N', null);";
                $stmt      = $pdo->query($req);
            } else
            {
        ...
        case "O":
            ...
            if ($methode == 'vendre' && $nombre > 9)
            {
                ...
                $perso->perso_po = $perso->perso_po + 200;
                $perso->stocke();
                $req  =
                    "select insere_evenement($perso_cod, $perso_dame_cygne, 100, '[perso_cod1] a vendu dix fleurs à la Dame Cygne.', 'O', 'N', null);";
                $stmt = $pdo->query($req);
            }
