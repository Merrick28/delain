	...
	switch($methode)
	{
		case "debut":
			...
			else
			{
				...
				{
					$obj_cod = $result['obj_cod'];
					$choix_flacon .= "<input type='radio' name='flacon' value='$obj_cod' id='$obj_cod'> <label for='$obj_cod'><a href='$PHP_SELF?&tpot=$tpot&methode=detail&fiole=$obj_cod'>" . $result['obj_nom'] . '</a> (' . $result['nombre'] . ' composants)</label><br />';
				}			
			}
			...
			if ($result[0] == 1)	// en cas de réussite de la compétence
			{
				...
				while($result2 = $stmt2->fetch())
				{
					...
					if ($result3 = $stmt3->fetch())
						$erreur_nombre = ($result3['nombre'] != $nombre2);	// Le composant entre dans la formule, mais il n’y est pas dans la bonne quantité
