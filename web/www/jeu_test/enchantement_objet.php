...
//On regarde les objets enchantables du perso en fonction de leur type, ainsi que si ils sont identifiés
        ...
        $contenu_page .= '<p align="left"><strong>Liste des objets qui doivent être manipulés pour devenir enchantables : </strong>
													<table>';
        $req4 = $req . ' and obj_enchantable = 0 and gobj_tobj_cod not in (6)
										order by gobj_tobj_cod,obj_nom desc';           // Marlyza 2019-04-09 - Cas particulier: les artefacts non enchantable, ne doivent pas le devenir.
        $db3->query($req4);
        if ($db3->nf() == 0) {
