...
ob_start();
?>
<title>EDITION D'UNE POTION</title>
	...
	switch($methode)
	{
		...
		case "ajout":
			...
			<input type="hidden" name="methode" value="ajout2">
				<tr>
					<td class="soustitre2">Nom / Description de la formule de potion (conserver le nom de la potion dedans)</td>
		...
		case "ajout2":
			...
			<br><br>Chaque potion contient un élément "base", à prendre parmi les suivants, en plus des autres composants : Pissenlit de vin (Composant base), Léonine sucrée (Composant base), Herbe de Lune (Composant base)
			<br><hr>
			<?php 
			if(!isset($action))
				$action = '';
		...
		case "modif":
			...
			?>
			<a href="<?php echo $PHP_SELF;?>?methode=serie_obj&pot=<?php echo $pot;?>">Modifier la liste d'objets</a><br>
			<table>
			<form name="ajout" method="post" action="<?php echo $PHP_SELF;?>">
		...
		case "modif2":
				...
				?><a href="<?php echo $PHP_SELF;?>?methode=serie_obj&pot=<?php echo $pot;?>">Modifier la liste des composants de la potion</a><br>
				<?php 
			break;
		case "serie_obj":
			?>
