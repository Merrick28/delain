...
switch ($methode)
{
	...
	case 'defi_lance2':  // Lancement d’un défi, envoi de message et création en base.
		...
		if ($defi_message == '')
			...
			$defi_message = 'Votre adversaire a également tenu à vous faire parvenir le message suivant : <hr />' . base_delain::format($defi_message);
		...
		if ($resultat_lancement[0] == 0)
			$avertissement = '<p style="color: darkred; font-weight: bold;">Attention ! ' . $resultat_lancement[1] . '</p>';
...
if ($db->get_one_record($req))
{
	...
	switch ($defi_statut)
	{
		case 0: $statut_texte = 'Défi lancé';
			if (!$is_lanceur)
...
function afficherDefisPasses($req_defis, $is_lanceur)
{
	...
	while ($db->next_record())
	{
		...
		switch ($defi_statut)
		{
			...
			case 5: $statut_texte = 'Match nul'; break;
		}
