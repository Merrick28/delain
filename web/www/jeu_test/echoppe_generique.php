...
<?php if(TYPE_ECHOPPE != "MAGIE"){
 ...
 <p> Cette échoppe est gérée par <b><?php echo $db->f("perso_nom")?></b> (<a href="javascript:document.message.submit();">Envoyer un message</a>)
<?php  }
	if ($controle_gerant == 'OK')
	{
...
?>
			...
			<p>Voulez-vous :</p>
			<ul>
			<li><a href="javascript:document.echoppe.affichage.value='acheter';document.echoppe.submit()">Acheter de l'équipement ?</a>
			<li><a href="javascript:document.echoppe.affichage.value='vendre';document.echoppe.submit()">Vendre de l'équipement ?</a>
		<?php if(TYPE_ECHOPPE != "MAGIE") {?>
			<li><a href="javascript:document.echoppe.affichage.value='identifier';document.echoppe.submit()">Faire identifier de l'équipement ?</a>
			<li><a href="javascript:document.echoppe.affichage.value='repare';document.echoppe.submit()">Faire réparer de l'équipement ?</a>
		<?php 
		...
		?><a href="javascript:document.echoppe.affichage.value='view_tran';document.echoppe.submit()">Voir les transactions ?</a>
					<?php  }
				 ?>
			</ul>
<?php 
...
if($db->nf() > 0){?>
<center>
<p>Les articles suivants sont en réserve, contactez le gérant de cette échoppe si vous souhaitez les acheter :</p>
<table>
<tr>
<td class="soustitre2"><p><b>Nom</b></td>
<td class="soustitre2"><p><b>Type</b></td>
...
<b>Près du comptoir se trouve une vitrine fermée à clé, c'est là qu'on trouve les objets exceptionnels. (Il faut bien sûr s'adresser au gérant pour en savoir un peu plus)</b><BR/>
<center>
<table>
<?php 
...
<?php 
  ...
  <table width="100%">
  <tr><td class="soustitre2">Objet</td><td class="soustitre2">Quantité</td><td class="soustitre2">Prix</td><td class="soustitre2">Accepter</td><td class="soustitre2">Refuser</td></tr>
 <?php 
...
$db->query($req_stock);
			while($db->next_record())
			{ ?>
		<tr><td class="soustitre2"><?php echo $db->f("gobj_nom")?></td><td class="soustitre2"><?php echo $db->f("tran_quantite")?></td><td class="soustitre2"><?php echo $db->f("tran_prix")?></td>
    <td class="soustitre2"><a href="javascript:document.valider_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.valider_tran.submit();">Accepter</a></td>
		<td class="soustitre2"><a href="javascript:document.cancel_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.cancel_tran.submit();">Refuser</a></td></tr>
			<?php 
			}
...
$db->query($req_stock);
			while($db->next_record())
			{ ?>
		<tr><td class="soustitre2"><?php echo $db->f("obj_nom")?></td><td class="soustitre2"><?php echo $db->f("tran_quantite")?></td><td class="soustitre2"><?php echo $db->f("tran_prix")?></td>
    <td class="soustitre2"><a href="javascript:document.valider_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.valider_tran.submit();">Accepter</a></td>
		<td class="soustitre2"><a href="javascript:document.cancel_tran.transaction_cod.value=<?php echo $db->f("tran_cod")?>;document.cancel_tran.submit();">Refuser</a></td></tr>
			<?php 
			}
