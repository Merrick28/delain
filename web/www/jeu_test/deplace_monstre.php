	...
	switch($methode2)
	{
		case 'recuperation':
			...
			else
			{
				...
				$req = "select ppos_perso_cod from perso_position, positions 
		...
		case 'validation': //Validation des monstres et validation de la position de lancement 
			...
			if ($erreur !=1)
			{
				...
				for($cpt = 0; $cpt < $nb_monstre; $cpt++)
				{
					...
					if($code != null)
					{
						$req = "select perso_nom, perso_type_perso, perso_cod, perso_sta_combat, perso_sta_hors_combat, perso_dirige_admin
		...
		case 'deplacement': //déplacement des monstres
			...
			foreach($x as $key => $val)
			{
				if($key != null)
				{
					...
					$req = "select pos_cod, etage_arene from positions 
					...
					{ 
						...
						// origine
						$req = "select pos_cod, etage_arene from perso_position
						...
						if ($gestion_ia)
						{
							...
							$req = "update perso set perso_dirige_admin = '$hors_ia',
