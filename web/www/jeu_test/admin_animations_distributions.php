...
switch ($methode)
{
	case 'distribution_creation':	// Crée une distribution d’objets
		...
		if ($code_race != 'tous')
		{
			$req_race = "select race_nom from race where race_cod = $code_race";
			$db->query($req_race);
			$db->next_record();
			$nom_race = ' de race « ' . $db->f('race_nom') . ' »';
		...
		{
			$req_monstre = "select gmon_nom from monstre_generique where gmon_cod = $code_monstre";
			$db->query($req_monstre);
			$db->next_record();
			$nom_monstre = ' de type « ' . $db->f('gmon_nom') . ' »';
		...
		else if ($code_etage != 'tous' && $antres)
		{
			...
			$req_etage = "select etage_libelle from etage where etage_numero = $code_etage";
			$db->query($req_etage);
			$db->next_record();
		...
		$req = "INSERT INTO historique_animations(anim_date, anim_texte, anim_type) values (now()::date, '$texte', 'distribution')";
		$db->query($req);
...
<input type="checkbox" name="antres" id="distrib_antres" checked="checked" /><label for="distrib_antres">Inclure les antres reliées à l’étage ?</label></td>

<td class="soustitre2">
	Distribution... <select name="distrib_localisation"><option value='inv'>En inventaire</option><option value='sol'>Au sol</option></select><br />
	<input type='checkbox' name="distrib_type_1" id='distrib_type_1' value='1' /><label for="distrib_type_1">Aventuriers</label><br />
	<input type='checkbox' name="distrib_type_2" id='distrib_type_2' value='2' /><label for="distrib_type_2">Monstres</label><br />
	<input type='checkbox' name="distrib_type_3" id='distrib_type_3' value='3' /><label for="distrib_type_3">Familiers</label><br />
	<select name="code_race"><option value="tous">Race indifférente</option>
<?php 
// Races
...
while ($db->next_record())
{
	echo '<option value="' . $db->f('race_cod') . '">' . $db->f('race_nom') . '</option>';
}
...
<td class="soustitre2">
	<label for="distrib_quantite">Quantité : </label><input type="text" name="distrib_quantite" id="distrib_quantite" value="" style="width:20px;" /><br />
	<label for="distrib_eparpillement">Éparpillement : 1 objet pour </label><input type="text" name="distrib_eparpillement" id="distrib_eparpillement" value="" style="width:20px;" /><label for="distrib_eparpillement"> cases.</label>
</td>
<td class="soustitre2">
