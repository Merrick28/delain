...
echo '<form name="deplacement" method="post" action="' . $action. '" target="droite">';
echo '<input type="hidden" name="methode" value="deplacement">';
...
for (i=0; i<tc.length; i++)
{
	...
	texte = texte + '<div id="dep" class="main" onClick="javascript:document.det_cadre.position.value=\'' + tc[i][0] + '\';document.det_cadre.dist.value=\'' + tc[i][8] + '\';document.det_cadre.submit();" title="' + titre + '">';
	...
	texte = texte + '<div id="cell' + tc[i][0] + '" class="pasvu" style="background:url(\'' + img_path + 'c1.gif\')" onClick="javascript:document.deplacement.position.value=\'' + tc[i][0] + '\';document.deplacement.submit();" title="' + titre + '">\r\n';
