    ...
    switch ($methode)
    {
        case 'entree':
            ...
            */
            ...
            $req  =
                'select tfid_perso_cod, perso_nom from perso, temple_fidele where tfid_perso_cod = perso_cod and tfid_lieu_cod = ' . $lieu;
            $stmt = $pdo->query($req);
            if ($result = $stmt->fetch())
            {
                ...
                <p>Ce temple est sous la responsabilité de <strong><?php echo $result['perso_nom'] ?></strong> (<a
                            href="javascript:document.message.submit();">Envoyer un message</a>)</p><br>
                <?php
            }
        ...
        else
        {
            ...
            $req                  =
                "SELECT count(*) count FROM perso_familier INNER JOIN perso ON perso_cod=pfam_familier_cod WHERE pfam_perso_cod = {$perso_cod} AND perso_actif='O' ";
            $stmt                 = $pdo->query($req);
            $result               = $stmt->fetch();
        ...
        ?>
            ...
            while ($result = $stmt->fetch())
            {
                ...
                if (strlen($description_longue) > 100)
                    $description_courte =
                        substr($description_longue, 0, 100) . "... <a href='javascript:permutterDescription($objet);'>(Voir plus)</a>";
        ...
        case 'grade':
            ...
            switch ($niveau)
            {
                case 1:
                    // novice
                    ?>
                    <p>En devenant novice, vous acceptez de rentrer dans la religion. Vous commencez à porter les signes distinctifs liés à votre religion (les autres joueurs auront donc dans la vue votre grade et votre divinité).
                    <br>
                    Vous gagnez la confiance de votre dieu, qui vous offre la possibilité de l’invoquer pour un sort mineur. Au moment ou vous voulez lancer ce sort, si votre dieu possède assez de puissance, il le lancera pour vous (pas de jet de dés et pas de gain d’expérience sur les sorts divins).
                    <br><br>
                    <?php
                    break;
                case 2:
                    // initié
                    ?>
                    <p>En devenant <?php echo $result['dniv_libelle']; ?>, vous faites un pas de plus dans la religion. Vous acceptez votre divinité pour seule et unique source de puissance.
                    <br>
                    Vous gagnez de nouveaux sorts divins, mais en retour, il vous est impossible de lancer des sorts mémorisés. Vous pouvez toujours utiliser les runes pour lancer des sorts non divins, mais vous ne faites pas de jet de mémorisation. Votre capacité de mémorisation n’augmentera pas pour ces sorts.
                    <br>
                    À partir de ce rang, vous avez la possibilité de prier votre divinité en extérieur, et non plus seulement dans un temple.
                    <br>
                    <?php
                    break;
                case 3:
                    // clerc
                    ?>
                    <p>Le grade de <?php echo $result['dniv_libelle']; ?> marque un tournant dans la religion. La confiance entre vous et votre dieu est maintenant totalement réciproque. C’est pour cela que votre dieu vous permet à tout moment d’avoir accès à la liste des personnes renégates.
                    <br>
        ...
        case "tel1":
            ...
            <table>
                ...
                while ($result = $stmt->fetch())
                {
                    ...
                    ?>
                    <tr>
                        <td class="soustitre2"><?php echo $result2['lieu_nom']; ?></td>
                        <td><?php echo $result2['pos_x']; ?>, <?php echo $result2['pos_y']; ?>
                            , <?php echo $result2['etage_libelle']; ?></td>
                        <td class="soustitre2">
                            <?php
                            if ($tab_lieu['pos_cod'] != $result[0])
                            {
                                ?>
                                <a href="action.php?methode=teld&pos=<?php echo $result[0]; ?>">Téléportation
                                    (<?php echo $cout_pa; ?> PA)</a>
                                <?php
                            }
                            ?>
                        </td>
        ...
        case 'benediction':
            if ($niveau_actu >= 2)
            {
                ...
                $req  =
                    "update dieu set dieu_pouvoir = dieu_pouvoir - " . $points_ben . " where dieu_cod = " . $dieu_cod;
                $stmt = $pdo->query($req);
                // on enlève les PA
                ...
                switch ($dieu_cod)
                {
                    ...
                    case 7:    // FAlis => Fou furieux
                        ...
                        $stmt  = $pdo->query($req);
                        $texte =
                            "Falis vous entend, et répond à votre demande de bénédiction. Vous vous sentez plus rapide au combat.";
                        break;
