...
function afficherResultats(obj) {
     ...
     if (tabResult.length > 0) {
          ...
          sel.setAttribute('value','parchemin');
          sel.setAttribute('name','parchemin');
					sel.setAttribute('id','parchemin');          
					sel.setAttribute('onchange','document.getElementById("parchemin2").value=document.getElementById("parchemin").value;');  
         	var egt = document.createElement('option');  
         var texte = document.createTextNode('<--Sélectionner un parchemin réalisable-->');
          egt.appendChild(texte); 
          egt.setAttribute('value', '-1'); 
         sel.appendChild(egt);
         for (var i = 0; i < tabResult.length; i++) {
               ...
               egt.setAttribute('value', resultat.getAttribute('valeur'));              
         			 sel.appendChild(egt);
          }
          ...
          document.getElementById('zonetexte').style.display = 'none';
     }
}
...
function loadData() {
     ...
     XHR.appendData("perso_cod", document.getElementById('perso').value);
...
function loadData2() {
     // Création de l'objet
     var XHR = new XHRConnection();
     XHR.appendData("foo", document.getElementById('groupe').value);
     // On soumet la requête
     // Signification des paramètres:               
     //      + On indique à l'objet qu'il faut appeler le fichier sadsearch2.php
     //      + On utilise la méthode POST, adaptée l'envoi d'information
     //      + On indique quelle fonction appeler lorsque l'opération a été effectuée
     XHR.sendAndLoad("sadsearch5.php", "POST", afficherResultats2);
    }
