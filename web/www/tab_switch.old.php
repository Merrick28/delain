...
function affiche_perso($perso_cod)
{
	...
	$req = "select perso_cod,perso_nom,to_char(perso_dlt,'DD/MM/YYYY hh24:mi:ss') as dlt,perso_energie,
		...
		pos_x,pos_y,pos_etage,perso_niveau,perso_avatar,etage_libelle,perso_description,perso_tangible,
        exists(select levt_cod from ligne_evt where levt_perso_cod1 = $perso_cod and levt_lu = 'N' limit 1) as events, perso_gmon_cod
	...
	$impalpable = ($db->f('perso_tangible') == 'N') ? '<br /><i>Impalpable</i>' : '';
	...
	echo '</td></tr>
		<tr><td valign="top"><center><a href="#" onClick="javascript:document.login.perso.value=' . $num_perso . ';document.login.submit();"><img src="' . $avatar . '" alt="Jouer ' . $db->f("perso_nom") . '"/></a>
	...
	if ($nb_msg != 0)
	{
		echo $nb_msg . ' messages non lus.<br>';
	}
	//
	// Transactions
	//
...
for ($cpt=0;$cpt<$nb_perso_max;$cpt++)
{
	...
	if ($cpt>=$nb_perso)
	{
		...
		echo '<table width="100%" height="100%" border="0">
			...
			<tr><td height="100%" valign="center">&nbsp;<br></td></tr>
			<tr><td><center><a href="' , $type_flux . G_URL , 'cree_perso_compte.php?compt_cod=' . $compt_cod . '"><img src="' . G_IMAGES . 'noperso.gif" alt="Créer un nouveau"></a></center></td></tr>';
