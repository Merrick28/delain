CREATE OR REPLACE FUNCTION public.nv_magie_purification(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- soin sur le lanceur
	select into v_pv,v_pv_max,v_voie_magique perso_pv,perso_pv_max,perso_voie_magique from perso
		where perso_cod = cible;
...
-- 	insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,5*ln(v_pv_cible));
	code_retour := code_retour||'<br>'||nom_cible||' a un bonus de '||trim(to_char(v_int_perso,'99'))||' en regénération pendant trois tours et n''a plus de malus de maladie ni de poison.<br>';
