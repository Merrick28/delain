CREATE or replace FUNCTION public.nv_magie_oeil_lynx(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_oeil_lynx : lance le sort oeil de lynx         */
...
/*   05/12/2007 : intégration de la MAJ automap                  */
/*****************************************************************/
declare
-------------------------------------------------------------
...
-------------------------------------------------------------	
    ...
    -- temps rallongé pour le maitre des arcanes
    if v_voie_magique = 2 then
        perform ajoute_bonus(cible, 'VUE', 4, 2);
        code_retour := code_retour || '<br>' || nom_cible || ' gagne un bonus de +2 en vue pendant 4 tours.<br>';
    else
        perform ajoute_bonus(cible, 'VUE', 2, 2);
        code_retour := code_retour || '<br>' || nom_cible || ' gagne un bonus de +2 en vue pendant 2 tours.<br>';
    end if;
...
/*On met à jour l'automap du perso ciblé*/
    perform update_automap(cible);
    ---------------------------
...
ALTER FUNCTION public.nv_magie_oeil_lynx(integer, integer, integer) OWNER TO delain;
