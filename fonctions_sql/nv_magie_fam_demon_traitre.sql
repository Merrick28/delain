CREATE OR REPLACE FUNCTION public.nv_magie_fam_demon_traitre(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
begin
	...
	where pfam_perso_cod = cible
		...
		and perso_actif = 'O';
	if found then
		code_retour := '<p>Malgré la réussite de votre invocation rien ne se passe. Il est impossible d''invoquer un familier alors que vous en avez déjà un.'||to_char(v_fam,'99999999999');
		return code_retour;
	end if;

	select into pos_lanceur ppos_pos_cod from perso_position where ppos_perso_cod = lanceur;
	-- id du demon: Parametre 111
	select into v_fam parm_valeur from parametres, perso where parm_cod = 111 and parm_valeur = perso_cod;
	if not found then
	...
	select into v_ancien_proprio, v_nom_ancien_proprio, v_pos_ancien_proprio, v_vie_image
			pfam_perso_cod, per.perso_nom, ppos_pos_cod, per.perso_int
	from perso_familier
	...
	if found then
		...
		insert into perso_familier (pfam_perso_cod, pfam_familier_cod, pfam_duree_vie) values (v_ancien_proprio, v_image_familier, v_vie_image);
	end if;
