CREATE OR REPLACE FUNCTION public.cree_objet_pos(integer, integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function cree_objet_pos  :Procédure de création d objet       */
/*                           sur une position donnée             */
/*                                                               */
/* On passe en paramètre :                                       */
/*   $1 = le gobj_cod (objet   générique)                        */
/*   $2 = Le pos_cod où l'objet doit apparaître                  */
/* Le code sortie est :                                          */
/*    0 = Tout s est bien passé                                  */
/*   -1 = pos_cod n existe pas                                   */
/*****************************************************************/
/* Liste des modifications :                                     */
/*****************************************************************/
declare
	code_retour integer;
	v_gobj alias for $1;
	v_level alias for $2;

-- variables globales
	compt integer;
	v_code_objet integer;
	retour_cree_perso integer;
	texte text;
begin
/**********************************************/
/* Etape 1 : on insère dans perso les valeurs */
/**********************************************/
	code_retour := 0;
	v_code_objet := nextval('seq_obj_cod');
	select into compt gobj_cod from objet_generique
		where gobj_cod = v_gobj;
	if not found then
		code_retour := -1;
		return code_retour;
	end if;
	insert into objets (obj_cod,obj_gobj_cod) values (v_code_objet,v_gobj);
		if v_gobj in ('723','829') then
			select into texte obj_text_texte from potions.objet_texte where obj_text_gobj_cod = v_gobj order by random() limit 1;
			update objets set obj_description = texte where obj_cod = v_code_objet;

		end if;
	
/*************************/
/* Etape 4 : on le place */
/*************************/
	insert into objet_position (pobj_cod,pobj_pos_cod,pobj_obj_cod) 
		values (nextval('seq_pobj_cod'),v_level,v_code_objet);
	code_retour := v_code_objet;
return code_retour;
end;
 
    
$function$

