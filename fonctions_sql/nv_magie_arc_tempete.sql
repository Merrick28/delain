CREATE or replace FUNCTION public.nv_magie_arc_tempete(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_arc_tempete: lance le sort arc de tempête      */
...
-- on vérifié qu'il n’y ait pas une arme
    ...
    insert into objets (obj_cod, obj_gobj_cod, obj_portee) values (num_objet, 106, 4);
    ...
    code_retour :=
                code_retour || '<br>Vous avez dans votre main un arc de tempête. Celui ci existera pendant 48 heures.';
...
ALTER FUNCTION public.nv_magie_arc_tempete(integer, integer, integer) OWNER TO delain;
COMMENT ON FUNCTION public.nv_magie_arc_tempete(integer, integer, integer) IS 'Lance le sort Arc de tempête';
