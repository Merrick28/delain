--
-- Name: valeur_bonus(integer, text); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION valeur_bonus(integer, text) RETURNS numeric
    LANGUAGE plpgsql
...
begin
  ...
  if v_type in ('DEX', 'INT', 'FOR', 'CON')  THEN
    ...
    where corig_perso_cod = v_perso and corig_type_carac = v_type LIMIT 1;
    if not found then
      v_retour := 0;
    end if;
  ...
  -- autres bonus que les caracs!
    ...
    select into v_retour coalesce(sum(bonus_valeur), 0) from bonus where bonus_perso_cod = v_perso and bonus_tbonus_libc = v_type;
    if not found then
      v_retour := 0;
    end if;
...
ALTER FUNCTION public.valeur_bonus(integer, text) OWNER TO delain;