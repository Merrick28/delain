CREATE or replace FUNCTION public.dv_mg_hargne(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_mg_aimant : lance le sort Hargne                  */
...
-------------------------------------------------------------
    ...
    v_pv_cible               integer;
    v_dieu                   integer; -- dieu de la cible
-------------------------------------------------------------
...
-------------------------------------------------------------	
    ...
    -- on vérifie que la cible soit bien un membre du clergé de balgur
    select into v_dieu dper_dieu_cod
    from dieu_perso
    where dper_perso_cod = cible;
    if not found or v_dieu <> 2 then
        code_retour := code_retour ||
                       'Votre sort est sans effet ! Votre cible n''est pas du clergé de Balgur, vous avez donc puisé dans les ressources divines pour rien !<br>';
        return code_retour;
    else
        perform ajoute_bonus(cible, 'PAA', 2, -1);
        perform ajoute_bonus(cible, 'DEG', 2, 4);
        perform ajoute_bonus(cible, 'ARM', 2, -4);
        code_retour := code_retour ||
                       'La cible a un bonus de -1Pa par attaque, +4 de dégats, et un malus de -4 en armure.<br>';

        code_retour :=
                    code_retour || '<br>Vous gagnez ' || trim(to_char(px_gagne, '999')) || ' PX pour cette action.<br>';
        ...
        values (3, lanceur, cible, 3 * ln(v_pv_cible));
    end if;
    return code_retour;
end;
$_$;
...
ALTER FUNCTION public.dv_mg_hargne(integer, integer, integer) OWNER TO delain;
