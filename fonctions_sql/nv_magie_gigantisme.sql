CREATE OR REPLACE FUNCTION public.nv_magie_gigantisme(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
select into nom_cible,v_pv_cible,taille_cible perso_nom,perso_pv_max,perso_taille from perso
...
if v_voie_magique = 5 then
  compt := compt + 1;
  code_retour := code_retour||'<br>Votre connaissance des enchantements vous permet d''acroitre plus vite la taille de votre cible.''<br>';
end if;
...
-- on traite en limitant la variation à la différence de taille jusqu'à 10
  ...
  degats:= degats * compt;
  code_retour := code_retour||'<br>Vous agrandissez la taille réelle de votre adversaire, lui infligeant des dégats insuportables '||nom_cible||'<br>';
  ...
  code_retour := code_retour||'<p>'||nom_cible||' subit un total de <b>'||trim(to_char(degats,'9999'))||'</b> dégats.<br>';
  texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible], effectuant '||trim(to_char(degats,'999'))||' dégats.';
  ...
  insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,0.25*degats*ln(v_pv_cible));
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,4*ln(v_pv_cible));
...
code_retour := code_retour||'<p>'||nom_cible||' est bien trop grand pour grandir encore. Votre sort est sans effet<br>';
return code_retour;
end;$function$
