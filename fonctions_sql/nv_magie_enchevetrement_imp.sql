CREATE or replace FUNCTION public.nv_magie_enchevetrement_imp(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
--------------------		   
    ...
    perform ajoute_bonus(cible, 'PAA', 3, v_pa_attaque);
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus au toucher de ' ||
                   trim(to_char(v_pa_attaque, '99')) || 'PA pendant 3 tours.';
...
ALTER FUNCTION public.nv_magie_enchevetrement_imp(integer, integer, integer) OWNER TO delain;
