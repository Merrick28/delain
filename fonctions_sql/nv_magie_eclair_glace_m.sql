CREATE OR REPLACE FUNCTION public.nv_magie_eclair_glace_m(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
v_bonus_pa_dep := 1;
code_retour := code_retour||'<br>Un mage de bataille comme vous fait toujours plus de dégats avec un éclair de glace que n''importe quel autre mage, et le malus lié au déplacement est toujours plus efficace.<br>';
end if;
...
degats := des;
code_retour := code_retour||'L''éclair en frappant sa cible fait <b>'||trim(to_char(degats,'99'))||'</b> dégats ';
...
update perso set perso_pv = perso_pv - degats where perso_cod = cible;
	...
	if ajoute_bonus(cible, 'DEP', 3, v_bonus_pa_dep) != 0 then
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,ln(v_pv_cible));
	end if;
	code_retour := code_retour||'<br>'||nom_cible||' a un malus au déplacement de '||trim(to_char(v_bonus_pa_dep,'99'))||'PA pendant 3 tours.';
      --  insert into -- ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
    -- 	values(nextval('seq_levt_cod'),14,now(),1,lanceur,texte_evt,'O','O',lanceur,cible);
     --   insert into -- ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
    -- 	values(nextval('seq_levt_cod'),14,now(),1,cible,texte_evt,'N','O',lanceur,cible);
