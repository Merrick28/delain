CREATE or replace FUNCTION public.nv_magie_malediction(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
    select into nom_cible,v_pv_cible,v_int_cible perso_nom, perso_pv_max, perso_int
    from perso
    ...
    v_chance_toucher := v_niveau_lanceur * 2;
    if v_chance_toucher > 40 then
        v_chance_toucher := 40;
    end if;
...
------------------------
        ...
        v_maladie := v_int_perso + 4;
        v_poison := v_int_perso + 4;
    else
        --------------------
-- magie résistée --
--------------------		   
        v_chance_toucher := round(v_chance_toucher / 2);
        v_pa_attaque := +1;
        v_malus_degats := 0 - 1;
        v_maladie := floor(v_int_perso / 2) + 2;
        v_poison := floor(v_int_perso / 2) + 2;
        code_retour := code_retour || 'Votre adversaire résiste partiellement au sort.<br>';
    end if;
    v_chance_toucher := 0 - v_chance_toucher;

    perform ajoute_bonus(cible, 'PAA', 2, v_pa_attaque);
    perform ajoute_bonus(cible, 'TOU', 2, v_chance_toucher);
    perform ajoute_bonus(cible, 'DEG', 2, v_malus_degats);
    perform ajoute_bonus(cible, 'MAL', 4, v_maladie);
    perform ajoute_bonus(cible, 'VEN', 4, v_poison);

    insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee)
    values (2, lanceur, cible, 5 * ln(v_pv_cible));
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus de ' || trim(to_char(v_pa_attaque, '99')) ||
                   'PA par attaque, ' || ' a un venin de ' || trim(to_char(v_poison, '999')) ||
                   ', a une maladie de puissance ' || trim(to_char(v_maladie, '999')) || ',' ||
                   trim(to_char(v_chance_toucher, '999999')) || ' chances de toucher, et ' ||
                   trim(to_char(v_malus_degats, '99999')) || ' aux dégats pendant 2 tours.';
