CREATE OR REPLACE FUNCTION public.nv_magie_malediction(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
	select into nom_cible,v_pv_cible,v_int_cible perso_nom,perso_pv_max,perso_int from perso
...
insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,5*ln(v_pv_cible));
	code_retour := code_retour||'<br>'||nom_cible||' a un malus de '||trim(to_char(v_pa_attaque,'99'))||'PA par attaque, '||' a un venin de '||trim(to_char(v_poison,'999'))||', a une maladie de puissance '||trim(to_char(v_maladie,'999'))||','||trim(to_char(v_chance_toucher,'999999'))||' chances de toucher, et '||trim(to_char(v_malus_degats,'99999'))||' aux dégats pendant 2 tours.';
