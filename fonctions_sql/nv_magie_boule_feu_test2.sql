CREATE OR REPLACE FUNCTION public.nv_magie_boule_feu_test2(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- NOMBRE DE CIBLES MAX = INT/2
	select into v_niv_lanceur perso_int from perso
		where perso_cod = lanceur;
	v_niv_lanceur := v_niv_lanceur /2;
  	code_retour := code_retour||'<b> Intelligence/2 = '||v_niv_lanceur||' Cibles au maximum</b> (cible principale comprise)<br>';
