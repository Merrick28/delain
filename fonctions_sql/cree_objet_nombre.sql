CREATE OR REPLACE FUNCTION public.cree_objet_nombre(integer, integer, integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
/*   $3 = le nombre d'objet de ce type à créer                   */
/* Le code sortie est :                                          */
...
/************************************************/
	...
	for compt in 1..v_nombre loop
		...
		insert into objet_position (pobj_cod,pobj_pos_cod,pobj_obj_cod) 
			values (nextval('seq_pobj_cod'),pos_aleatoire(v_level),v_code_objet);
	end loop;	
return 0;
end;
