CREATE or replace FUNCTION public.nv_magie_portail_flammes(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- OK on créé l'élémentaire	
    v_monstre := cree_monstre_pos(185, v_pos);
    v_monstre := cree_monstre_pos(185, v_pos);
    v_monstre := cree_monstre_pos(185, v_pos);
    v_monstre := cree_monstre_pos(185, v_pos);
    v_monstre := cree_monstre_pos(185, v_pos);
    ...
    v_monstre := cree_monstre_pos(185, v_pos);
    v_monstre := cree_monstre_pos(185, v_pos);
    code_retour := code_retour || '<br>Vous avez invoqué avec succès 10 élémentaires de feu.';
...
ALTER FUNCTION public.nv_magie_portail_flammes(integer, integer, integer) OWNER TO delain;
