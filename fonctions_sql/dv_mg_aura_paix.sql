CREATE or replace FUNCTION public.dv_mg_aura_paix(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function aura_paix : lance le sort                            */
...
-------------------------------------------------------------	
    ...
    update perso
    set perso_dlt = perso_dlt + (to_char((round(perso_temps_tour / 2)), '99999999') || ' minutes')::interval
    where perso_cod = lanceur;
    code_retour :=
                code_retour || 'Tous les blocages de combat de ' || nom_cible || ' en tant que cible ont disparu.<br>';
...
ALTER FUNCTION public.dv_mg_aura_paix(integer, integer, integer) OWNER TO delain;
