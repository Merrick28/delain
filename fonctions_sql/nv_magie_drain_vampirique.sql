CREATE OR REPLACE FUNCTION public.nv_magie_drain_vampirique(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	select into v_perso_niveau,v_perso_int perso_niveau,perso_int from perso
...
--------------------		   
	...
	select into pv_cible perso_pv from perso
		where perso_cod = cible;
	insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,2*ln(v_pv_cible));
