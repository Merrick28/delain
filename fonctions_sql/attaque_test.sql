...
/*   31/05/2007 : modif des dégats pvp initiative                */
...
declare
  ...
  chance_touche_autre integer;
  -- chance de toucher un autre adversaire
  des_autre integer;
  ...
  v_nb_des_degats integer;	-- des de dégats par défaut
  ...
  comp_attaque_modifie integer;
  -- valeur de la comp d'attaque modifiée
  nb_des_attaque integer;		-- nb de dés de dégats utilisés pour l'attaque
  valeur_des_attaque integer;
  -- valeur des des de dégats
  ...
  limite_comp_maitre integer;
  -- limite pour le coup spécial
  ...
  temp_comp_attaque numeric;
  -- texte temporaire pour l'amel de la compétence
  v_chute numeric;				-- chute de l'arme
  v_amel_degats_dist integer;
  -- bonus aux dégats à distance
  ...
  -- nombre de fois où la compétence est utilisable
...
begin
  ...
  debug_txt := debug_txt||'Modificateur de limite pour coup spécial (et. 1): '||trim(to_char(v_limite_maitre,'99D99'))||'.<br>';
  -- initialisation du texte debug
  debug_txt := debug_txt||'Valeur facteur 1 : '||trim(to_char(v_facteur1,'99999999'))||'<br>'||'Valeur facteur 2 : '||trim(to_char(v_facteur2,'99999999'))||'<br>';
  v_malus_surcharge := 0;
  v_cible_surcharge := 0;
  ...
  if v_type_attaque = 0 then
    ...
    num_comp_spe := 66;
    v_cout_pa := nb_pa_attaque(v_attaquant) + 3;
    nom_type_attaque := ' (Coup de grace) ';
  elsif v_type_attaque = 8 then
    num_comp_spe := 67;
    v_cout_pa := nb_pa_attaque(v_attaquant) + 1;
    nom_type_attaque := ' (Coup de grace lvl 2) ';
  elsif v_type_attaque = 9 then
    num_comp_spe := 68;
    v_cout_pa := nb_pa_attaque(v_attaquant);
    nom_type_attaque := ' (Coup de grace lvl 3) ';
  ...
  /**************************************************/
  debug_txt := debug_txt||'Compétence initiale (liée à l''arme): '||trim(to_char(comp_attaque_init,'99999999999999'))||'.<br>';
  debug_txt := debug_txt||'Chance initiale (sorts, concentration, armes, type d''attaque) : '||trim(to_char(comp_attaque,'99999999999999'))||'.<br>';
  ---------------------------------------------------------------------------
  -- introduction de la surcharge
  comp_attaque := comp_attaque - v_malus_surcharge;
  debug_txt := debug_txt||'Chance (Après surcharge) : '||trim(to_char(comp_attaque,'99999999999999'))||'.<br>';
  ---------------------------------------------------------------------------
  ...
  debug_txt := debug_txt||'Modif en fonction du type de combat : '||trim(to_char(v_modif_att,'99D99'))||'.<br>';
  debug_txt := debug_txt||'Chance 4 (Après mode de combat + facteur 1) : '||trim(to_char(comp_attaque,'99999999999999'))||'.<br>';
  ---------------------------------------------------------------------------
  ...
  if v_type_arme = 2 then
    ...
    debug_txt := debug_txt||'Chance 5 (si arme de distance, en fonction de la case) : '||trim(to_char(comp_attaque,'99999999999999'))||'.<br>';
  end if;
  /**************************************************/
  ...
  debug_txt := debug_txt||'Défense 1 (brut, compétence de la cible) : '||trim(to_char(c_def,'99999999999999'))||'.<br>';
  ...
  debug_txt := debug_txt||'Modificateur de défense lié à la différence d''arme : '||trim(to_char(m_def,'99D99'))||'.<br>';
  debug_txt := debug_txt||'Défense 2 (avec modificateur) : '||trim(to_char(c_def,'99999999999999'))||'.<br>';
  ------------------------------------------
  ...
  debug_txt := debug_txt||'Modificateur de type de combat : '||trim(to_char(v_modif_def,'99D99'))||'.<br>';
  debug_txt := debug_txt||'Défense final (avec facteur 2) : '||trim(to_char(c_def,'99999999999999'))||'.<br>';
  --
  ...
  debug_txt := debug_txt||'Chance de toucher (attaque / défense) : '||trim(to_char(comp_attaque,'99999999999999'))||'.<br>';
  /********************************************************************************/
  ...
  debug_txt := debug_txt||'Modificateur de limite pour coup spécial : '||trim(to_char(v_limite_maitre,'99D99'))||'.<br>';
  ...
  if des > 96 then -- echec critique
    ...
    if v_type_arme != 0 then
      update objets
      set obj_etat = obj_etat - (usure_arme * 2)
      where obj_cod = num_arme;
      select into etat_arme
        obj_etat from objets
      where obj_cod = num_arme;
      if etat_arme <= 0 then
        ...
        values(37,now(),v_attaquant,texte_evt,'N','O');
    ...
    values(nextval('seq_levt_cod'),9,now(),1,v_attaquant,texte_evt,'O','O',v_attaquant,nv_cible);
    insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
    values(nextval('seq_levt_cod'),9,now(),1,nv_cible,texte_evt,'N','O',v_attaquant,nv_cible);
    ...
    if ((v_type_attaque < 13) or (v_type_attaque > 15)) then
      if des_autre < chance_touche_autre then
        ...
        select into nv_cible,nom_per2,pv_cible,v_armure_physique,armure
          perso_cod,perso_nom,perso_pv,f_armure_perso_physique(nv_cible),f_armure_perso(nv_cible)
        ...
        values(nextval('seq_levt_cod'),35,now(),1,v_attaquant,texte_evt,'O','O',v_attaquant);
        insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant)
        values(nextval('seq_levt_cod'),35,now(),1,nv_cible,texte_evt,'N','O',v_attaquant);
      end if;
    end if;
    /*********************************************/
  ...
  if valeur_bonus(nv_cible, 'JUS') != 0 then
    if des <= 5 then
      ...
      delete from bonus
      where bonus_perso_cod = nv_cible
      ...
      values(47,now(),v_attaquant,texte_evt,'O','N',v_attaquant,nv_cible);
    end if;
  end if;
  if des <= 5 then
    if v_armure_physique != 0 then
      ...
      if v_type_arme = 2 then
        update objets
        set obj_etat = obj_etat - usure_arme
        where obj_cod = num_arme;
        select into etat_arme
          obj_etat from objets
        where obj_cod = num_arme;
    ...
    -- etape 5.5 : on calcule les degats portés
    /*v_bonus_critique integer;
    v_des_critique integer;*/
    ...
    if v_bonus_critique != 0 then
      ...
      if v_des_critique < v_bonus_critique then
        ...
        values(47,now(),v_attaquant,texte_evt,'O','N',v_attaquant,nv_cible);
        insert into ligne_evt(levt_tevt_cod,levt_date,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
        values(47,now(),nv_cible,texte_evt,'N','O',v_attaquant,nv_cible);
  ...
  /*********************************************/
    ...
    if v_armure_physique != 0 then
      if v_type_arme != 0 then
        update objets
        set obj_etat = obj_etat - (usure_arme * 1.5)
        where obj_cod = num_arme;
        select into etat_arme
          obj_etat from objets
        where obj_cod = num_arme;
    ...
    if getparm_n(105) = 0 then
      ...
      select into v_perso_nb_spe
        perso_nb_spe
      from perso
      where perso_cod = v_attaquant;
      update perso
      set perso_nb_spe = perso_nb_spe + 1
      where perso_cod = v_attaquant;
      px_gagne := px_gagne + (1/v_perso_nb_spe::numeric);
    end if;
  ...
  /*********************************************/
    ...
    if nb_hon != 0 then
      code_retour := code_retour||'Grace au sort honneur, le coup devient un coup spécial.';
      qualite_attaque := 1;
    end if;
    code_retour := code_retour||'<br><br>';
  ...
  if reussite_esquive != 0 then -- esquive cible réussie
    ...
    if var_gain = 1 then
      ...
      values (1,v_attaquant,nv_cible,max(0,(degats_portes - armure + 5)));
    end if;
    code_retour := code_retour||'<hr><b>Informations de débuggage : </b><br><i>'||debug_txt||'</i><br><hr>';
    return code_retour;
  else
  ...
  if v_type_arme != 2 then
    ...
    if v_aura_feu != 0 then
      ...
      code_retour := code_retour||'<br>L''aura de feu de votre adversaire vous provoque <b>'||trim(to_char(v_degats_aura_feu,'99999999'))||'</b> dégats.<br>';
      if v_degats_aura_feu > v_pv_attaquant then
        ...
        update perso
        set perso_pv = perso_pv - v_degats_aura_feu where perso_cod = v_attaquant;
      end if;
      code_retour := code_retour||'<br>';
      texte_evt := 'L''aura de feu de [attaquant] a causé '||trim(to_char(v_degats_aura_feu,'9999'))||' dégats à [cible].';
      insert into ligne_evt(levt_tevt_cod,levt_date,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
      values(46,now(),v_attaquant,texte_evt,'O','N',nv_cible,v_attaquant);
      insert into ligne_evt(levt_tevt_cod,levt_date,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
      values(46,now(),nv_cible,texte_evt,'N','N',nv_cible,v_attaquant);
    end if;
  end if;
  /****************************/
  /* FIN   : aura de feu      */
  ...
  /*****************************************/
  /* DEBUT : coup porté : ajout des dégats */
  /*****************************************/
  ...
  code_retour := code_retour||'et vous portez donc '||trim(to_char(degats_effectues,'9999'))||' points de dégats.<br>'; -- pos 14
  etat_armure := f_use_armure(v_cible,degats_portes);
  if degats_effectues is null then
    degats_effectues := 0;
  end if;
  --
  ...
  if v_degats_effectues != degats_effectues then
    code_retour := code_retour||'<br>Les dégats rééls liés à l''initiative sont de '||trim(to_char(v_degats_effectues,'999999999')) || '.<br />';
  ...
  if coalesce(v_vampire,0) > 0 then
    ...
    code_retour := code_retour||'Vous avez régénéré '||trim(to_char(regen_vampire,'9999999'))||' points de vie grace au vampirisme.<br>';
  end if;
  ...
  /*****************************************/
  /* FIN   : coup porté : ajout des dégats */
  /*****************************************/
  --
  /*****************************************/
  ...
  if nouveau_pv_cible <= 0 then -- la cible a été tuée......
    ...
    texte_evt := '[attaquant] a frappé [cible] '||nom_arme||nom_type_attaque||', infligeant '||trim(to_char(degats_effectues,'9999'))||' points de dégats.'||nom_qualite_attaque;
  ...
  /*****************************************/
    ...
    values(nextval('seq_levt_cod'),9,now(),1,nv_cible,texte_evt,'N','O',v_attaquant,nv_cible);
    /*insert into action
      (act_tact_cod,act_perso1,act_perso2,act_donnee)
      values
      (4,nv_cible,v_attaquant,degats_effectues);*/
    update perso
