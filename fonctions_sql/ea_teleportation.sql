--
-- Name: ea_teleportation(integer, integer, integer, character, text, numeric, text, json); Type: FUNCTION; Schema: public; Owner: delain
--

create or replace function ea_teleportation(integer, integer, integer, character, text, numeric, text, json) RETURNS text
LANGUAGE plpgsql
AS $_$/**************************************************/
/* ea_teleportation                             */
...
begin
  ...
  for ligne in (select perso_cod , perso_type_perso , perso_race_cod, perso_nom, perso_niveau, perso_int, perso_con, pos_cod, perso_pv
                ...
                where perso_actif = 'O'
                      ...
                      ((v_cibles_type = 'S' and perso_cod = v_source) or
                       ...
                       (v_cibles_type = 'T'))
                -- cas spécifique de la téléportation:
                      and (perso_type_perso!=3)                                                                         -- on ne téléporte pas les familiers
  ...
  loop
      ...
      if v_pos_teleportation > 0 then
          code_retour := code_retour || '<br />'|| ' « téléportation »  de ' || ligne.perso_nom || '.' ;
          ...
          update perso_position set ppos_pos_cod = v_pos_teleportation where ppos_perso_cod = ligne.perso_cod;
          ...
          if found then
              update perso_position	set ppos_pos_cod = v_pos_teleportation  where ppos_perso_cod = v_perso_fam;
...
ALTER FUNCTION public.ea_teleportation(integer, integer, integer, character, text, numeric, text, json) OWNER TO delain;
