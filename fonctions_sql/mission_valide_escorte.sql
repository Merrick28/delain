CREATE OR REPLACE FUNCTION public.mission_valide_escorte(integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*************************************************************/
/* fonction mission_valide_escorte                           */
...
begin
	...
	if v_vivant and v_pos_cod_cible = ligne.mpf_pos_cod then
		...
		perform insere_evenement(ligne.mpf_perso_cod, ligne.mpf_perso_cod, 93,
			'[perso_cod1] a escorté un émissaire dans le cadre d’une mission', 'O', '[mpf_cod]=' || code_mission::text);
	end if;
