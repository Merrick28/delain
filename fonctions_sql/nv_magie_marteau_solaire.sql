CREATE or replace FUNCTION public.nv_magie_marteau_solaire(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_marteau_solaire : lance le sort épée de feu           */
...
-- on vérifié qu'il n'y ait pas une arme
    ...
    insert into objets (obj_cod, obj_gobj_cod) values (num_objet, 603);
    ...
    code_retour :=
                code_retour || '<br>Vous avez dans votre main un marteau solaire. Celui ci existera pendant 48 heures.';
...
ALTER FUNCTION public.nv_magie_marteau_solaire(integer, integer, integer) OWNER TO delain;
