--
-- Name: ramasse_or(integer, integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION public.ramasse_or(integer, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*****************************************************************/
...
/**************************************/
select into pos_perso,nom_perso,v_pos_etage ppos_pos_cod,perso_nom,pos_etage from perso_position,perso,positions
...
code_retour := code_retour || execute_fonctions(personnage, null, 'DEP', json_build_object('type_dep','ramasse_or', 'qte_or', qte_or, 'ancien_pos_cod',pos_perso,'ancien_etage',v_pos_etage,'nouveau_pos_cod',pos_perso,'nouveau_etage',v_pos_etage)) ;
...
ALTER FUNCTION public.ramasse_or(integer, integer) OWNER TO delain;
