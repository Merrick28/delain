CREATE or replace FUNCTION public.nv_magie_mercu_groupe(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-------------------------------------------------------------	
    ...
    where ppos_pos_cod = v_pos
      ...
      and perso_actif = 'O';
    for ligne in
        select perso_cod, perso_nom, perso_pv_max, perso_sex
        from perso,
             perso_position
        where ppos_pos_cod = v_pos
          and ppos_perso_cod = perso_cod
        ...
        loop
            ...
            -- malus := valeur_bonus(ligne.perso_cod, 'MAL');
            compt := 1;
            temp_bonus := 0;
            ...
            code_retour := code_retour || '<br>' || ligne.perso_nom || ' a regagné ' ||
                           trim(to_char(amel_pv, '9999')) || ' points de vie.<br>';
...
ALTER FUNCTION public.nv_magie_mercu_groupe(integer, integer, integer) OWNER TO delain;
