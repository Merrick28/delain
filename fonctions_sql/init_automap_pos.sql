CREATE OR REPLACE FUNCTION public.init_automap_pos(integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$declare
	code_retour integer;
	...
	test2 integer;
begin
	delete from donnees_automap where dauto_pos_cod = v_pos;	
	for ligne in select * from positions where pos_cod = v_pos loop
		test_donnees := 0;
		test2 := 0;
test_donnees := 0;
		test2 := 0;
