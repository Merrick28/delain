CREATE or replace FUNCTION public.nv_magie_danse_guy(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-------------------------------------------------------------	
    ...
    code_retour := code_retour || '<br>' || nom_cible || ' a un bonus de ' || trim(to_char(bonus_esquive, '99999')) ||
                   ' pendant 2 tours. Durant 2 tours il est aussi considéré en mode défensif';
...
ALTER FUNCTION public.nv_magie_danse_guy(integer, integer, integer) OWNER TO delain;
