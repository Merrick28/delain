CREATE OR REPLACE FUNCTION public.nv_magie_impotence(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
	select into nom_cible,v_pv_cible,v_taille perso_nom,perso_pv_max,perso_taille from perso
...
--------------------		   
	...
	code_retour := code_retour||'<br>'||nom_cible||' a un malus de 20%, pendant 2 tours, pour ses chances de coups spéciaux. Cela est du à votre pouvoir de mage de guerre ';
        end if;
	...
	code_retour := code_retour||'<br>'||nom_cible||' a un malus à l''esquive de '||trim(to_char(v_pa_attaque,'99999'))||' pendant 2 tours.';
