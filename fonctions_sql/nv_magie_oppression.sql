CREATE OR REPLACE FUNCTION public.nv_magie_oppression(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
--------------------		   
		...
		code_retour := code_retour||'Votre adversaire résiste partiellement au sort.<br>';
			...
			code_retour := code_retour||'<br>Vous lancez le sort <b>'||nom_sort||'</b> sur '||nom_cible||', faisant '||trim(to_char(v_degats,'99'))||'<br> !';
