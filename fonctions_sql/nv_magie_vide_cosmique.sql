CREATE or replace FUNCTION public.nv_magie_vide_cosmique(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_vide cosmique : lance le vide cosmique         */
/*                                                               */
...
-------------------------------------------------------------	
    ...
    select into resultat vide_cosmique(v_pos, 1);
    code_retour := code_retour || 'Vous avez lancé avec succés le sort vide cosmique<br>' || resultat;
...
ALTER FUNCTION public.nv_magie_vide_cosmique(integer, integer, integer) OWNER TO delain;
