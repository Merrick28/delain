CREATE OR REPLACE FUNCTION public.dv_mg_attention_divine(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,3*ln(v_pv_cible));
	code_retour := code_retour||'<br>'||nom_cible||' a son nombre d''esquive qui est repassé à 0 pour ce tour.<br>';
