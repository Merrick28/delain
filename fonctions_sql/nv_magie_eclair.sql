CREATE OR REPLACE FUNCTION public.nv_magie_eclair(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
	...
	-- on regarde si bloque magie
		...
		if proj_bloque = 0 then -- sort non bloqué
			if resiste_magie(cible,lanceur,niveau_sort) = 0 then
				...
				code_retour := code_retour||'que votre adversaire arrive à repousser partiellement. Il ne subit que '||trim(to_char(degats,'99'))||' dégats.<br>';
			end if;
		end if;
