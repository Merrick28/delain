CREATE or replace FUNCTION public.dv_mg_malchance_totale(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- on enlève les bonus existants
    perform ajoute_bonus(cible, 'MAE', 2, v_bonus);
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus de ' || trim(to_char(v_bonus, '999999')) ||
...
ALTER FUNCTION public.dv_mg_malchance_totale(integer, integer, integer) OWNER TO delain;
