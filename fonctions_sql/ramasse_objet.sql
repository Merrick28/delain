--
-- Name: ramasse_objet(integer, integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION public.ramasse_objet(integer, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*****************************************************************/
...
code_retour text;
	...
	gobj integer;   -- code du genrique d’objet
	tobj integer;   -- code du type d’objet
...
begin
	...
	/* On cherche le perso                */
	/**************************************/
select into tangibilite, v_poids_max, pos_perso, v_poids_actu, pa, v_pos_etage
    perso_tangible, perso_enc_max, ppos_pos_cod, get_poids(perso_cod), perso_pa, pos_etage
from perso
...
return code_retour;
	...
	/* On cherche l’objet                 */
	/**************************************/
select into nom_objet, tobjet, gobj, v_poids_objet, pos_objet, objet_position_cod, tobj
    obj_nom_generique, tobj_libelle, gobj_cod, obj_poids, pobj_pos_cod, pobj_cod, tobj_cod
from objets
...
code_retour := code_retour || execute_fonctions(personnage, null, 'DEP', json_build_object('type_dep','ramasse_objet', 'gobj_cod', gobj, 'tobj_cod', tobj, 'ancien_pos_cod',pos_perso,'ancien_etage',v_pos_etage,'nouveau_pos_cod',pos_perso,'nouveau_etage',v_pos_etage)) ;
...
ALTER FUNCTION public.ramasse_objet(integer, integer) OWNER TO delain;

--
-- Name: FUNCTION ramasse_objet(integer, integer); Type: COMMENT; Schema: public; Owner: delain
--

COMMENT ON FUNCTION public.ramasse_objet(integer, integer) IS 'Gère le ramassage d''un objet par un perso.';
