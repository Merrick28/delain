CREATE OR REPLACE FUNCTION public.nv_magie_transfert_energie(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- on récupère les infos
	select into v_pv,v_pv_max perso_pv,perso_pv_max from perso
       ...
       select into v_pv_lanceur,v_pv_max_lanceur,v_int_lanceur,v_niv_lanceur,v_voie_magique perso_pv,perso_pv_max,perso_int,perso_niveau,perso_voie_magique from perso
...
-- sauf si le lanceur est moins blessé que sa cible
	...
	select into nom_cible,sexe_cible perso_nom,perso_sex from perso
		where perso_cod = cible;
        select into nom_lanceur,sexe_cible perso_nom,perso_sex from perso
		where perso_cod = lanceur;
...
if pourcent_blessure > pourcent_blessure_lanceur then
   ...
   temp_bonus := floor(v_int_lanceur *1.75);
         ...
         code_retour := code_retour||'<br>Votre connnaissance de guérisseur accroit votre puissance dans ce  sort.<br>';
...
perform soin_compteur_pvp(lanceur);
	...
	amel_pv := nouveau_pv_lanceur - v_pv_lanceur;
        insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,0.25*ln(v_pv)*perte_pv);
        code_retour := code_retour||'<br>'||nom_cible||' a perdu '||trim(to_char(perte_pv,'9999999'))||' points de vie.<br>';	
	code_retour := code_retour||'<br>'||nom_lanceur||' a regagné '||trim(to_char(amel_pv,'9999999'))||' points de vie.<br>';
	...
	code_retour := code_retour||'<br>Vous gagnez '||px_gagne||' PX pour cette action.<br>';
		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible], lui faisant perdre '||trim(to_char(perte_pv,'9999'))||' points de vie.';
