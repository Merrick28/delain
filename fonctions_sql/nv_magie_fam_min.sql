CREATE or replace FUNCTION public.nv_magie_fam_min(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_fam_min : lance le sort                        */
...
/*   31/10/2007 : Modif du choix des sorts.                      */
...
-------------------------------------------------------------	
    ...
    -- on créé le familier
    select into v_temp ajoute_familier(191, cible);
...
ALTER FUNCTION public.nv_magie_fam_min(integer, integer, integer) OWNER TO delain;
COMMENT ON FUNCTION public.nv_magie_fam_min(integer, integer, integer) IS 'Invoque un familier mineur magique';
