CREATE OR REPLACE FUNCTION public.nv_magie_fam_min(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	where pfam_perso_cod = cible
		...
		and perso_actif = 'O';
	if found then
		code_retour := '<p>Malgré la réussite de votre invocation rien ne se passe. Il est impossible d’invoquer un familier alors que vous en avez déjà un.</p>';
		return code_retour;
	end if;
