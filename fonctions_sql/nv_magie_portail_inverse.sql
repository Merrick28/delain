CREATE OR REPLACE FUNCTION public.nv_magie_portail_inverse(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Pour la destination	
	...
	if found then
		code_retour := 'Malgré la réussite de votre incantation, rien ne se passe car un lieu se trouve sur la case de destination.<br>';
	...
	if found then
		code_retour := 'Malgré la réussite de votre incantation, rien ne se passe car un mur se trouve sur la case de destination.<br>';
		tmp_annule := annule_sort(lanceur,num_sort,type_lancer,px_gagne);
		return code_retour;
	end if;
	...
	if found then
		code_retour := 'Malgré la réussite de votre incantation, rien ne se passe car vous vous trouvez sur un lieu.<br>';
		tmp_annule := annule_sort(lanceur,num_sort,type_lancer,px_gagne);
		return code_retour;
	end if;
...
-- lieu destination
	...
	insert into lieu
		(lieu_cod,lieu_tlieu_cod,lieu_nom,lieu_description,lieu_refuge,lieu_url,lieu_dest,lieu_port_dfin)
		values
		(v_lieu,10,'Passage magique','Un passage crée par la magie...<br><br>&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;&#160;<img src="../avatars/passage_entree.gif"><br>','N','passage.php',pos_lanceur,now()+v_delai);
	insert into lieu_position (lpos_pos_cod,lpos_lieu_cod)
		values (v_pos,v_lieu);
-- automap
...
px_gagne := px_gagne +2;
code_retour := code_retour||'<br>Votre esprit de maître des arcanes vous permet de progresser plus rapidement que les autres magiciens ouvrant des passages.<br>';
end if;
