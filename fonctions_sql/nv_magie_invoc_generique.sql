CREATE OR REPLACE FUNCTION public.nv_magie_invoc_generique(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
 STRICT
AS $function$/*****************************************************************/
...
-------------------------------------------------------------        
        ...
        select into type_monstre, nombre_monstres, duree_vie
            invoc_monstre_gmon_cod, invoc_nombre, 
                case when invoc_duree = 0 then 999 else invoc_duree end -- 999 jours max.
            from invocation where invoc_lanceur_gmon_cod = 
        ...
        for compt in 1..nombre_monstres loop
            v_monstre := cree_monstre_pos(type_monstre,v_pos_cible);
            update perso set perso_cible = cible,perso_dfin = now() + 
                duree_vie * '1 day'::interval where perso_cod = v_monstre;
