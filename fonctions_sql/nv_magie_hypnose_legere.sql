CREATE OR REPLACE FUNCTION public.nv_magie_hypnose_legere(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
	select into nom_cible,v_pv_cible,v_pa perso_nom,perso_pv_max,perso_pa from perso
	 ...
	 code_retour := code_retour||'<br>'||nom_cible||' perd tous ses PA restants.';
        ...
        if v_voie_magique = 2 then
         -- on aliment le malus d'hypnoptisme pour les deux prochains tours de 2
            ...
            code_retour := code_retour||'Un effet d''hypnotisme supplémentaire vient affecter ' ||nom_cible||' , qui subira un malus de 3 PA en moins lors de ses deux prochaines réactivations.<br>';
            end if;
         else
         -- on aliment le malus d'hypnoptisme pour les deux prochains tours de 2
            if ajoute_bonus(cible, 'HYP', 2, 2) != 0 then
            -- on ajoute les bonus qui vont bien
            code_retour := code_retour||'Un effet d''hypnotisme supplémentaire vient affecter ' ||nom_cible||' , qui subira un malus de 2 PA en moins lors de ses deux prochaines réactivations.<br>';
            end if;
         end if;
else
         v_pa := v_pa - 8;
