CREATE or replace FUNCTION public.nv_magie_invoc_gelee(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_invoc_gelee : lance le sort invocation gelée   */
/*  magique                                                      */
...
-------------------------------------------------------------        
    ...
    v_monstre := cree_monstre_pos(46, v_pos_cible);
    ...
    code_retour := code_retour || 'Vous avez invoqué une gelée bleue qui attaque votre cible.<br>';
...
ALTER FUNCTION public.nv_magie_invoc_gelee(integer, integer, integer) OWNER TO delain;
