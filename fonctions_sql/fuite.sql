--
-- Name: fuite(integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION public.fuite(integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*****************************************************************/
/* function fuite                                                */						/*							         */
...
begin
	...
	code_retour := code_retour||'<br>Votre lancer de dés est de <b>'||trim(to_char(des,'9999'))||'</b>, ';
      ...
      -- Marlyza 2021-04-30 - il y a des déplacements possible à 1PA, la fuite coûte 2PA, on retire ce que l'on peut!
      update perso set perso_pa = GREATEST(0, perso_pa - getparm_n(19)) where perso_cod = num_perso;
...
ALTER FUNCTION public.fuite(integer) OWNER TO delain;