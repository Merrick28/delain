CREATE or replace FUNCTION public.nv_magie_elementaire_eau(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- OK on créé l'élémentaire	
    v_monstre := cree_monstre_pos(184, v_pos);
    code_retour := code_retour || '<br>Vous avez invoqué avec succès un élémentaire d''eau.';
...
ALTER FUNCTION public.nv_magie_elementaire_eau(integer, integer, integer) OWNER TO delain;
