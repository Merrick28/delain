CREATE OR REPLACE FUNCTION public.nv_magie_catalyse(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
begin
	...
	select into v_voie_magique, v_regen perso_voie_magique, perso_des_regen from perso
	...
	select into v_bonus_tour, v_bonus_valeur
		bonus_nb_tours, bonus_valeur
	from bonus
	where bonus_perso_cod = cible
		and bonus_valeur > 0
	...
	insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (2, lanceur, cible, 0.25 * v_regen * ln(v_pv_cible));

	code_retour := code_retour || ' Vous infectez le sang de votre cible. Ce poison (' || text_vregen || ') sera actif durant 3 tours.<br>';
	if degats = 0 then
		code_retour := code_retour || 'Votre cible n’avait ni poison, ni venin, ni sang infecté : votre sortilège est sans effets supplémentaires.<br>'; 
	else
		des := effectue_degats_perso(cible, degats, lanceur);
		if des != degats then
			code_retour := code_retour || '<br>Les dégâts réels liés à l’initiative sont de ' || trim(to_char(des,'999999999')) || '.<br />';
			insert into trace (trc_texte) values ('att ' || trim(to_char(lanceur, '99999999')) || ' cib ' || trim(to_char(cible, '99999999')) || ' init ' || trim(to_char(degats,'99999999')) || ' fin ' || trim(to_char(des,'99999999')));
		end if;
		degats := des;
		code_retour := code_retour || 'La catalyse provoque une alchimie dans le corps de votre cible qui fait <b>' || trim(to_char(degats, '999')) || '</b> dégâts.';
		...
		code_retour := code_retour || '<p>' || nom_cible || ' subit un total de <b>' || trim(to_char(degats, '9999')) || '</b> dégâts.<br>';
	
		insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (2, lanceur, cible, 0.25 * degats * ln(v_pv_cible));
	...
	texte_evt := '[attaquant] a lancé ' || nom_sort || ' sur [cible], effectuant ' || trim(to_char(degats, '999'))||' dégâts.';
