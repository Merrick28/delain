CREATE OR REPLACE FUNCTION public.nv_magie_boule_feu(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------
	...
	v_res_feu integer;
        ...
        v_bonus_degats integer;                              -- bonus dégats lié à la voie magique
...
-- on prend la position
	...
	select into nb_cible count(perso_cod)
        ...
        -- tout d'abord on determine si le lanceur est un mage de bataille pour ces différents bonus
        v_bonus_degats := 0;
        v_bonus_cible := 0;
        if v_voie_magique = 6 then
        v_bonus_degats := floor(v_niv_lanceur/25);
        v_bonus_cible := floor(v_niv_lanceur/20);
        end if;
...
-- magie non résistée --
------------------------
		v_degats := lancer_des(6,5);
                v_degats := v_degats + 2 + v_bonus_degats;
		code_retour := code_retour||'Votre adversaire n''arrive pas à résister au sort.<br>';
	else
--------------------
-- magie résistée --
--------------------
		v_degats := lancer_des(2,5);
                v_degats := v_degats + 2 + v_bonus_degats;
...
insert into trace (trc_texte) values ('att '||trim(to_char(lanceur,'99999999'))||' cib '||trim(to_char(cible,'99999999'))||' init '||trim(to_char(v_degats,'99999999'))||' fin '||trim(to_char(des,'99999999')));
...
v_degats := des;
	...
	code_retour := code_retour||'Vous provoquez '||trim(to_char(v_degats,'99999'))||' dégats sur <b>'||nom_cible||'</b><br>';
...
-- NOMBRE DE CIBLES MAX = INT/2
	v_int_lanceur := floor(v_int_lanceur /2);
        -- ajout du bonus mage de bataille
        v_int_lanceur := v_int_lanceur + v_bonus_cible;
        v_int_lanceur_txt := v_int_lanceur;
  	code_retour := code_retour||'<b> Intelligence/2 = '||v_int_lanceur_txt||' Cibles au maximum</b> (cible principale comprise et bonus mage de bataille inclus)<br>';
	...
	for ligne in select perso_cod,perso_nom,perso_pv,perso_pv_max,lancer_des(1,1000) as num
		...
		and perso_cod != cible 
                order by num limit v_int_lanceur loop
		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible]';
		v_degats := lancer_des(2,5);
                -- dégats différents pour le mage de bataille
                if v_voie_magique = 6 then
...
v_degats := des;
	...
	-- on regarde si esquive
		...
		-- on a tué l'adversaire !!
			px_gagne := px_gagne + to_number(split_part(tue_perso_final(lanceur,ligne.perso_cod),';',1),'9999999999999999');
