CREATE OR REPLACE FUNCTION public.nv_magie_mini_portail(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- OK on créé l'élémentaire	
	...
	for num_monstre in 1..nb_monstre loop
		v_monstre := cree_monstre_pos(61,v_pos);
	end loop;
	code_retour := code_retour||'<br>Vous avez invoqué avec succès '||trim(to_char(nb_monstre,'99999'))||' diablotins.';
