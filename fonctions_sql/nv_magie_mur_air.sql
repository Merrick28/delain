CREATE OR REPLACE FUNCTION public.nv_magie_mur_air(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_defense : lance le sort mur d'air              */
...
-------------------------------------------------------------	
        select into v_perso_int,v_voie_magique perso_int,perso_voie_magique from perso 
                ...
                v_perso_int := floor(v_perso_int *1.3);
        if v_voie_magique = 6 then 
                v_perso_int := v_perso_int + 8;
        else
	...
	perform ajoute_bonus(cible, 'MUR', 3, v_perso_int);
	code_retour := code_retour||'<br>'||nom_cible||' est maintenant protégé par un mur d''air rendant bien plus facile ses esquives.<br>';
