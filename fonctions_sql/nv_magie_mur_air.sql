CREATE or replace FUNCTION public.nv_magie_mur_air(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-------------------------------------------------------------	
    ...
    code_retour := code_retour || '<br>' || nom_cible ||
                   ' est maintenant protégé par un mur d''air rendant bien plus facile ses esquives.<br>';
...
ALTER FUNCTION public.nv_magie_mur_air(integer, integer, integer) OWNER TO delain;
