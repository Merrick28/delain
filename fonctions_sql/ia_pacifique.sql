--
-- Name: ia_pacifique(integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE or REPLACE FUNCTION ia_pacifique(integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*****************************************************/
...
/*************************************/
	if v_cible is null then
		code_retour := code_retour||'Pas de cible départ, déplacement aléatoire.<br>';
		if statique_hors_combat = 'N' then
			compt_loop := 0;
			while (v_pa >= getparm_n(9)) loop
				compt_loop := compt_loop + 1;
...
ALTER FUNCTION public.ia_pacifique(integer) OWNER TO delain;
