CREATE OR REPLACE FUNCTION public.nv_magie_frayeur(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,5*ln(v_pv_cible));
	code_retour := code_retour||'<br>'||nom_cible||' a un malus de '||trim(to_char(v_pa_attaque,'99'))||'PA par attaque, '||trim(to_char(v_chance_toucher,'999999'))||' chances de toucher, et '||trim(to_char(v_malus_degats,'99999'))||' aux dégats pendant 2 tours.';
