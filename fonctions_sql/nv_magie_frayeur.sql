CREATE or replace FUNCTION public.nv_magie_frayeur(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_frayeur : lance le sort frayeur                */
...
-------------------------------------------------------------	
    ...
    select into v_niveau_lanceur perso_niveau
    from perso
    where perso_cod = lanceur;
    v_chance_toucher := v_niveau_lanceur * 5;
    if v_chance_toucher > 25 then
        v_chance_toucher := 25;
    end if;
...
--------------------		   
    ...
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus de ' || trim(to_char(v_pa_attaque, '99')) ||
                   'PA par attaque, ' || trim(to_char(v_chance_toucher, '999999')) || ' chances de toucher, et ' ||
...
ALTER FUNCTION public.nv_magie_frayeur(integer, integer, integer) OWNER TO delain;
