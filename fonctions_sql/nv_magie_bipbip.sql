CREATE or replace FUNCTION public.nv_magie_bipbip(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_bipbip : lance le sort bip-bip                 */
...
-- on enlève les bonus existants
    ...
    code_retour := code_retour || '<br>' || nom_cible || ' a un bonus de -2 en déplacement pendant 2 tours.<br>';
...
ALTER FUNCTION public.nv_magie_bipbip(integer, integer, integer) OWNER TO delain;
