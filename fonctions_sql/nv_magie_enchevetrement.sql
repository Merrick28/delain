CREATE OR REPLACE FUNCTION public.nv_magie_enchevetrement(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------
	...
	code_retour := code_retour || '<br>' || nom_cible || ' a un malus au toucher de ' || trim(to_char(v_pa_attaque, '99')) || 'PA pendant 2 tours.';
