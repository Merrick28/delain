CREATE or replace FUNCTION public.nv_magie_enchevetrement(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_enchevetrement : lance le sort enchevêtrement  */
...
-------------------------------------------------------------
    ...
    if ajoute_bonus(cible, 'PAA', 2, v_pa_attaque) != 0 then
    ...
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus au toucher de ' ||
                   trim(to_char(v_pa_attaque, '99')) || 'PA pendant 2 tours.';
...
ALTER FUNCTION public.nv_magie_enchevetrement(integer, integer, integer) OWNER TO delain;
COMMENT ON FUNCTION public.nv_magie_enchevetrement(integer, integer, integer) IS 'Fonction gérant le sort Enchevêtrement';
