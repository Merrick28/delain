CREATE OR REPLACE FUNCTION public.nv_magie_conscience_flux(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	if v_voie_magique = 1 then
             compt := compt + 2;
             code_retour := code_retour||' En tant que guerisseur votre conscience des flux de soins est bien supérieure à celle des autres mages.<br>';
	end if;
