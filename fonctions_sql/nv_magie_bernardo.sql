CREATE or replace FUNCTION public.nv_magie_bernardo(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_bernardo : lance le sort bernardo              */
...
--------------------		   
    ...
    perform ajoute_bonus(cible, 'BER', duree, 1);
    code_retour := code_retour || '<br>' || nom_cible || ' ne pourra pas envoyer de message pendant ' ||
                   trim(to_char(duree, '99')) || ' tours.<br>';
...
ALTER FUNCTION public.nv_magie_bernardo(integer, integer, integer) OWNER TO delain;
