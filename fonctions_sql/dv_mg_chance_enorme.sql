CREATE OR REPLACE FUNCTION public.dv_mg_chance_enorme(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- on enlève les bonus existants
	...
	insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,2.5*ln(v_pv_cible));
	code_retour := code_retour||'<br>'||nom_cible||' a un bonus de '||trim(to_char(v_bonus,'999999'))||' en esquive pendant 2 tours.<br>';
