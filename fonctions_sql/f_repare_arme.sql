...
begin
	...
	select into temp, is_identifie
		perobj_obj_cod, perobj_identifie
	from perso_objets
	...
	if v_etat >= min(v_capa_repar , 100) * v_etat_max / 100 then
		code_retour := '<p>Votre capacité de réparation ne vous permet pas d’améliorer l’état de cette arme.</p>';
		return code_retour;
	end if;
	...
	if v_des > 95 then -- échec critique
		...
		if v_etat <= 0 then -- destruction
			...
			if (v_pa >= 2*getparm_n(40)) then
				code_retour := code_retour||'<a href="action.php?methode=repare&type=1&objet='||cast(num_objet as varchar(20))||'">Réessayer ('||cast(getparm_n(40) as varchar(2))||' PA)</a><br>';
			end if;
	...
	if v_des <= 5 then -- réussite critique
		...
		update objets
		set obj_etat = obj_etat + round(v_capa_repar / 3 * 2)
		where obj_cod = num_objet;

		code_retour := code_retour || 'Vous avez <b>amélioré</b> l’état de l’objet. Il est maintenant dans un état ' || get_etat_objet(MIN(cast(v_etat + round(v_capa_repar / 3 * 2) as integer), v_etat_max)) || '.<br><br>';

		if (v_pa >= 2*getparm_n(40)) then
			code_retour := code_retour || '<a href="action.php?methode=repare&type=1&objet=' || cast(num_objet as varchar(20)) || '">Recommencer (' || cast(getparm_n(40) as varchar(2)) || ' PA)</a><br>';
		end if;
	...
	if v_des > 5 and v_des <= v_comp then -- réussite
		...
		update objets
		set obj_etat = obj_etat + round(v_capa_repar/3),
		...
		code_retour := code_retour||'Vous avez <b>amélioré</b> l’état de l’objet. Il est maintenant dans un état '||get_etat_objet(MIN(cast(v_etat + round(v_capa_repar/3) as integer), v_etat_max))||'.<br><br>';
		if (v_pa >= 2*getparm_n(40)) then
			code_retour := code_retour||'<a href="action.php?methode=repare&type=1&objet='||cast(num_objet as varchar(20))||'">Recommencer ('||cast(getparm_n(40) as varchar(2))||' PA)</a><br>';
		end if;
		texte_evt := '[perso_cod1] a réparé un objet.';
...
ALTER FUNCTION public.f_repare_arme(integer, integer) OWNER TO delain;

--
-- Name: FUNCTION f_repare_arme(integer, integer); Type: COMMENT; Schema: public; Owner: delain
--

COMMENT ON FUNCTION public.f_repare_arme(integer, integer) IS 'Gère la réparation d’une arme.';