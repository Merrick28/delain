...
begin
  ...
  -- vérifier s'il y a un terrain specifique a cette position, on regarde si c'est une monture avec des caracs speciales sur ce terrain
  select m.perso_cod, tmon_event_chance, coalesce(tmon_event_pa, ''), tmon_message, ter_nom, p.perso_nom, m.perso_nom into v_monture, v_event_chance, v_event_pa, v_message, v_terrain, v_perso_nom, v_monture_nom
    from perso as p
  ...
  update perso set perso_pa = LEAST(GREATEST(0, perso_pa+valeur), 12) where perso_cod = v_perso ;
  ...
  if v_message != '' then
      ...
      if strpos(v_message , '[cible]') != 0 then
        perform insere_evenement(v_monture, v_perso, 54, v_message, 'O', 'N', null);
      else
        perform insere_evenement(v_monture, v_perso, 54, v_message, 'O', 'O', null);
      end if;
