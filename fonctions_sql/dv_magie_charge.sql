CREATE or replace FUNCTION public.dv_magie_charge(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-------------------------------------------------------------	
    ...
    if nb_lock != 0 then
        code_retour := code_retour ||
                       'Vous échouez dans votre sort divin. La charge divine ne fonctionne pas lorsque l''on est déja engagé en combat<br>';
        return code_retour;
    end if;
...
ALTER FUNCTION public.dv_magie_charge(integer, integer, integer) OWNER TO delain;
