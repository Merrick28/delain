CREATE or replace FUNCTION public.nv_magie_drain_vie(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
    select into nom_cible,v_pv_cible,v_for_cible perso_nom, perso_pv_max, perso_for
    from perso
...
-- bonus pour le sorcier
    if v_voie_magique = 3 then
        ...
        code_retour := code_retour ||
                       'votre pouvoir de sorcier vous permet d''augmenter la puissance du drain de vie.<br>';
    else
        v_bonus_sorcier := 0;
    end if;
...
--------------------		   
    ...
    values (2, lanceur, cible, 0.5 * ln(v_pv_cible) * drain_pv);
...
ALTER FUNCTION public.nv_magie_drain_vie(integer, integer, integer) OWNER TO delain;
