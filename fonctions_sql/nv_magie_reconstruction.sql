CREATE OR REPLACE FUNCTION public.nv_magie_reconstruction(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_reconstruction : lance le sort reconstruction  */
...
-------------------------------------------------------------	
	...
	px_gagne := split_part(magie_commun_txt,';',4);
	 ...
	 if v_voie_magique = 1 then
          ...
          compt := floor(compt/10);
          compt := compt + 5;

              code_retour := code_retour||' En tant que guérisseur votre efficacité sur ce sort est amélioré.<br>';
          end if;	
	...
	if ajoute_bonus(cible, 'REG', 4, compt) != 0 then
        ...
        compt_text := compt;
	code_retour := code_retour||'<br>'||nom_cible||' gagne '||compt_text||' en régénération pendant 4 tours.<br>';
