CREATE or replace FUNCTION public.nv_magie_incineration(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
    ...
    for ligne in select perso_cod, perso_nom, perso_pv
        ...
        loop
            ...
            if ligne.perso_pv <= v_degats then
                ...
                px_gagne := px_gagne +
                            to_number(split_part(tue_perso_final(lanceur, ligne.perso_cod), ';', 1), '9999999999999');
...
ALTER FUNCTION public.nv_magie_incineration(integer, integer, integer) OWNER TO delain;
