CREATE or replace FUNCTION public.nv_magie_soins_importants(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- on enlève les bonus existants
    ...
    from perso
    where perso_cod = cible;
    select into v_perso_niveau, v_voie_magique perso_niveau, perso_voie_magique
    from perso
...
-- on passe à l'augmentation de pvies
    ...
    if amel_pv > 0 then
        ...
        values (3, lanceur, cible, 5 * ln(amel_pv));
    end if;
...
ALTER FUNCTION public.nv_magie_soins_importants(integer, integer, integer) OWNER TO delain;
