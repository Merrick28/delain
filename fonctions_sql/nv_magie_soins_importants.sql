CREATE OR REPLACE FUNCTION public.nv_magie_soins_importants(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- on regarde si la personne est malade
   ...
   -- on regarde si le lanceur est sous conscience des flux de soin
	     ...
	     if v_bonus_flux > 0 then
             -- pour soins important le bonus est doublé
                v_bonus_flux := v_bonus_flux *2;
              ...
              code_retour := code_retour||'<br>Vous gagnez '||bonus_flux||' points de vie grace à la conscience des flux de soins.<br>';
              else
...
-- on passe à l'augmentation de pvies
 select into v_pv,v_pv_max perso_pv,perso_pv_max from perso
	...
	if amel_pv > 0 then
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,5*ln(amel_pv));
	end if;
