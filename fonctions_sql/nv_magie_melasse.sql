CREATE or replace FUNCTION public.nv_magie_melasse(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
--------------------		   
    ...
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus au déplacement de ' ||
                   trim(to_char(v_bonus_pa_dep, '99')) || 'PA pendant 2 tours.';
...
ALTER FUNCTION public.nv_magie_melasse(integer, integer, integer) OWNER TO delain;
