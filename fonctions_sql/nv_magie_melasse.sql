CREATE OR REPLACE FUNCTION public.nv_magie_melasse(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_melasse : lance le sort melasse                */
...
-------------------------------------------------------------
	...
	v_bonus_pa_dep integer;		-- Force du sort
-------------------------------------------------------------
...
--------------------		   
	...
	if ajoute_bonus(cible, 'DEP', 3, v_bonus_pa_dep) then
		
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,ln(v_pv_cible));
	end if;
	code_retour := code_retour||'<br>'||nom_cible||' a un malus au déplacement de '||trim(to_char(v_bonus_pa_dep,'99'))||'PA pendant 2 tours.';
