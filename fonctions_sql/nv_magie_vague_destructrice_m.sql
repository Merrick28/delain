CREATE OR REPLACE FUNCTION public.nv_magie_vague_destructrice_m(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_vague_destructrice_m : lance  vague destructrice */
...
/*   23/01/2013 : Création VD pour monstres                      */
/*****************************************************************/
declare
	-------------------------------------------------------------
...
begin
	...
	select into v_voie_magique, v_perso_int
			perso_voie_magique, perso_int
	from perso
	where perso_cod = lanceur;
	bonus_voie := 0;
	if v_voie_magique = 6 then
		bonus_voie := 8;
		code_retour:=code_retour||'<br> La puissance de votre vague destructrice est augmentée par votre connaissance de la magie de bataille.<br>';
	end if;
	...
	for ligne in select perso_cod, perso_nom, perso_pv, perso_pv_max,
			case when random() < immun_valeur and (immun_runes = 'O' or type_lancer <> 0) then 'O' else 'N' end as immunite,
			immun_valeur, immun_runes
		from perso
		inner join perso_position on ppos_perso_cod = perso_cod
		left outer join monstre_generique_immunite on immun_gmon_cod = perso_gmon_cod and immun_sort_cod = num_sort
		where perso_actif = 'O'
			and perso_type_perso = 1
			and perso_tangible = 'O'
			and perso_cod != lanceur
		...
		if (ligne.immunite = 'O') then
			...
			if ligne.immun_runes = 'O' and ligne.immun_valeur = 1 then
				code_retour := code_retour || '<b>' || ligne.perso_nom || '</b> est totalement immunisé à ce sort.<br><br>';
			end if;
			if ligne.immun_runes = 'O' and ligne.immun_valeur < 1 then
				code_retour := code_retour || '<b>' || ligne.perso_nom || '</b> est partiellement immunisé à ce sort, ce qui lui permet de s’en tirer sans dommage.<br><br>';
			end if;
			if ligne.immun_runes = 'N' and ligne.immun_valeur = 1 then
				code_retour := code_retour || '<b>' || ligne.perso_nom || '</b> est totalement immunisé à ce sort.<br>Peut-être devriez-vous essayer de composer ce sort avec des runes ?<br><br>';
			end if;
			if ligne.immun_runes = 'N' and ligne.immun_valeur < 1 then
				code_retour := code_retour || '<b>' || ligne.perso_nom || '</b> est partiellement immunisé à ce sort, ce qui lui permet de s’en tirer sans dommage.<br>Peut-être devriez-vous essayer de composer ce sort avec des runes ?<br><br>';
			end if;
			...
			v_degats := floor(lancer_des(2, 5) * v_perso_int / 8) + bonus_voie;
			...
			if reussite_esquive != 0 then
				...
				if ligne.perso_pv <= v_degats then
					...
					px_gagne := px_gagne + to_number(split_part(tue_perso_final(lanceur,ligne.perso_cod),';',1),'999999999999');
	...
	for ligne in select perso_cod,perso_nom,perso_pv,perso_pv_max
		...
		where perso_actif = 'O'
			...
			and not exists
				...
				and lpos_lieu_cod = lieu_cod
				and lieu_refuge = 'O')
				and pos_etage = e_lanceur limit 50 loop

		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible]';
		
		v_degats := floor(lancer_des(1, 4) * v_perso_int / 8) + bonus_voie;
		...
		if reussite_esquive != 0 then
			...
			if ligne.perso_pv <= v_degats then
				-- on a tué l'adversaire !!
				px_gagne := px_gagne + to_number(split_part(tue_perso_final(lanceur,ligne.perso_cod),';',1),'99999999999999999');
				code_retour := code_retour||'Vous avez <b>tué</b> '||ligne.perso_nom||'<br><br>';
			else
				code_retour := code_retour||ligne.perso_nom||' a survécu à votre attaque<br><br>';
				update perso set perso_pv = perso_pv - v_degats where perso_cod = ligne.perso_cod;
