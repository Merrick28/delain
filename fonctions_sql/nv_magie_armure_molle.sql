CREATE or replace FUNCTION public.nv_magie_armure_molle(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function armure_molle : lance le sort                         */
...
--------------------		   
    ...
    if ajoute_bonus(cible, 'ARM', 2, v_malus_armure) then

        insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee)
        values (2, lanceur, cible, 8 * ln(v_pv_cible));
    end if;
    code_retour := code_retour || '<br>' || nom_cible || ' a un malus d’armure de ' ||
                   trim(to_char(v_malus_armure, '99')) || ' pendant 2 tours.';
...
ALTER FUNCTION public.nv_magie_armure_molle(integer, integer, integer) OWNER TO delain;
COMMENT ON FUNCTION public.nv_magie_armure_molle(integer, integer, integer) IS 'Lance le sort Armure Molle';
