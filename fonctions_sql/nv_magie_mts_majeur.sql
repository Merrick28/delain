CREATE or replace FUNCTION public.nv_magie_mts_majeur(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-------------------------------------------------------------	
    ...
    select into v_perso_niveau,v_perso_int,v_voie_magique perso_niveau, perso_int, perso_voie_magique
    from perso
...
-- on ajoute l'effet de la voie magique
    if v_voie_magique = 5 then
        ...
        code_retour := code_retour ||
                       '<br>Vous usez de votre savoir d''enchanteur runique pour améliorer encore plus la précision de votre cible.''<br>';
        des := 0;
        ...
        if des < compt then
            nb_sort_tour := 3;
            code_retour :=
                        code_retour || '<br>Cette fois, votre intelligence vous a permis de prolonger le sort.''<br>';
        end if;
    end if;
...
ALTER FUNCTION public.nv_magie_mts_majeur(integer, integer, integer) OWNER TO delain;
