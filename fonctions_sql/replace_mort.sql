...
begin
    nouvelle_position := -1;
    select into etage_ref, v_arene, v_type_arene etage_mort,
                                                 etage_arene,
                                                 etage_type_arene
    from perso_position,
         positions,
    ...
    if v_mode = 0 then
        ...
        select into v_position rpos_pos_cod
        from perso_resuc,
             positions
        ...
        if found then
            ...
            perform f_del_objet(pglyphe_obj_cod)
            from perso_glyphes
            ...
            if nouvelle_position = -1 and found then
                ...
                if des_temple <= chance_temple then
                    ...
                    update perso_temple
                    set ptemple_nombre = ptemple_nombre + 1
                    where ptemple_perso_cod = v_cible;
                else
                    nouvelle_position := -1;
                end if;
    ...
    if v_mode = 1 then
        ...
        if found then
            update perso_temple
            set ptemple_anc_pos_cod = ptemple_pos_cod,
                ptemple_anc_nombre  = ptemple_nombre
            where ptemple_perso_cod = v_cible;

            update perso_temple
            set ptemple_pos_cod = nouvelle_position,
                ptemple_nombre  = - 50
            where ptemple_perso_cod = v_cible;
        else
            insert into perso_temple (ptemple_pos_cod, ptemple_nombre, ptemple_anc_pos_cod, ptemple_anc_nombre,
                                      ptemple_perso_cod)
            values (nouvelle_position, -50, 0, 0, v_cible);
        end if;
        texte_evt := '[attaquant] a jeté [cible] en prison';
        insert into ligne_evt(levt_tevt_cod, levt_date, levt_type_per1, levt_perso_cod1, levt_texte, levt_lu,
    ...
    texte_evt := '[cible] réapparait au niveau ' || trim(to_char(nouveau_etage, '999999')) || ', en position ' ||
                 trim(to_char(nouveau_x, '9999')) || ', ' || trim(to_char(nouveau_y, '9999')) || '.';
