CREATE OR REPLACE FUNCTION public.nv_magie_implosion(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
select into v_int_perso,v_voie_magique,v_niv_lanceur perso_int,perso_voie_magique,perso_niveau from perso where perso_cod = lanceur;
...
ratio := compt * 0.040;
code_retour := code_retour||'<br>La puissance du mage de guerre se reflète aujourd''hui dans la force de votre implosion.''<br>';
...
code_retour := code_retour||'<br>Vous faites imploser '||nom_cible||' profitant de sa taille pour lui faire des dégats internes immenses.''<br>';
