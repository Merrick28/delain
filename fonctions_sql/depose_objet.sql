--
-- Name: depose_objet(integer, integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION public.depose_objet(integer, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*****************************************************************/
...
code_retour text;                           -- code retour
	...
	v_tobj_cod objet_generique.gobj_tobj_cod%type;  -- le type d’objet à déposer
    v_gobj_cod integer;                               -- code du genrique d’objet
...
begin
	...
	/********************************************/
    select into pa, pos_perso, v_pos_etage
	...
	/*******************************************/
    select into nom_objet, v_tobj_cod, v_deposable, v_gobj_cod
        obj_nom_generique, gobj_tobj_cod, obj_deposable, gobj_cod
        from objets
    ...
    code_retour := code_retour || execute_fonctions(personnage, null, 'DEP', json_build_object('type_dep','depose_objet', 'gobj_cod', v_gobj_cod, 'tobj_cod', v_tobj_cod, 'ancien_pos_cod',pos_perso,'ancien_etage',v_pos_etage,'nouveau_pos_cod',pos_perso,'nouveau_etage',v_pos_etage)) ;
...
ALTER FUNCTION public.depose_objet(integer, integer) OWNER TO delain;

--
-- Name: FUNCTION depose_objet(integer, integer); Type: COMMENT; Schema: public; Owner: delain
--

COMMENT ON FUNCTION public.depose_objet(integer, integer) IS 'Gère le dépôt d’un objet au sol.';