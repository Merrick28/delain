CREATE OR REPLACE FUNCTION public.nv_magie_conscience_runique(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- on enlève les bonus existants
	if ajoute_bonus(cible, 'ERU', temp_ameliore_tour, -2) != 0 then
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,0.5*ln(v_pv_cible));
	end if;
	code_retour := code_retour||'<br>'||nom_cible||' gagne un bonus de -2PA par sort pendant '|| temp_ameliore_competence ||' tours, pour les sorts d’enchanteurs runiques.<br>';
