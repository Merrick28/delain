--
-- Name: ia_monture_speed(integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE or REPLACE FUNCTION ia_monture_speed(integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*****************************************************/
/* fonction ia_monture_speed                             */
/*    reçoit en arguments :                          */
...
/***********************************/
	...
	if found then
      ...
      if v_posm != v_posp then
          ...
          v_posc := trajectoire_projection(pos_actuelle, v_posm, v_posp, 1) ;
...
/***********************************/
  ...
  code_retour := code_retour||'Monture libre, déplacement aléatoire.<br>';
...
ALTER FUNCTION public.ia_monture_speed(integer) OWNER TO delain;
