CREATE or replace FUNCTION public.nv_magie_fam_min_dist(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_fam_min_dist : lance le sort                    */
...
-------------------------------------------------------------	
    ...
    -- on créé le familier
    select into v_temp ajoute_familier(193, cible);
...
ALTER FUNCTION public.nv_magie_fam_min_dist(integer, integer, integer) OWNER TO delain;
COMMENT ON FUNCTION public.nv_magie_fam_min_dist(integer, integer, integer) IS 'Invoque un familier mineur de combat à distance.';
