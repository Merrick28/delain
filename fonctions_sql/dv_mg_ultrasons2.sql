CREATE or replace FUNCTION public.dv_mg_ultrasons2(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- on enlève les bonus existants
    perform ajoute_bonus(cible, 'ULT', 2, v_bonus);
    insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (3, lanceur, cible, 2);
    code_retour := code_retour || '<br>' || nom_cible || ' entendra TRES mal pendant 2 tours.<br>';
...
ALTER FUNCTION public.dv_mg_ultrasons2(integer, integer, integer) OWNER TO delain;
