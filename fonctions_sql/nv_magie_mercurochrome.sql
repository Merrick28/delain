CREATE or replace FUNCTION public.nv_magie_mercurochrome(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_mercurochrome : lance le sort mercurochrome    */
...
-------------------------------------------------------------
    ...
    bonus_flux               text; -- valeur des gains de flux
-------------------------------------------------------------
-- variables concernant le lanceur	
-------------------------------------------------------------
    lanceur alias for $1; -- perso_cod du lanceur
    v_perso_niveau           integer; -- niveau du lanceur
    v_perso_type             integer; -- Type du lanceur(Joueur, Monstre, Familier)
-------------------------------------------------------------
...
-------------------------------------------------------------
    ...
    malus                    integer; -- malus de la maladie
    v_bonus_flux             integer; -- bonus pour conscience des flux
-------------------------------------------------------------
...
-- on enlève les bonus existants
    ...
    from perso
    where perso_cod = cible;
    select into v_perso_niveau, v_perso_type perso_niveau, perso_type_perso
    from perso
    where perso_cod = lanceur;
    compt := 1;
    temp_bonus := 0;
    if v_perso_niveau > 5 or v_perso_type = 3 then
        v_perso_niveau := 5;
    end if;
...
-- on regarde si la personne est malade
    ...
    if v_bonus_flux != 0 then
        -- on regarde si le lanceur et la cible sont la meme personne
        if lanceur = cible then
            ...
            code_retour := code_retour || '<br>Vous gagnez ' || bonus_flux ||
                           ' points de vie grâce à la conscience des flux de soins.<br>';
        else
...
-- on passe à l'augmentation de pvies de la cible
    select into v_pv,v_pv_max perso_pv, perso_pv_max
    from perso
    where perso_cod = cible;
    nouveau_pv := v_pv + temp_bonus;
    if nouveau_pv > v_pv_max then
        temp_bonus := v_pv_max - v_pv;
        nouveau_pv := v_pv_max;
    end if;
    update perso set perso_pv = nouveau_pv where perso_cod = cible;
    perform soin_compteur_pvp(cible);
    insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (3, lanceur, cible, 1);
    amel_pv := nouveau_pv - v_pv;
    code_retour := code_retour || '<br>' || nom_cible || ' a regagné ' || trim(to_char(amel_pv, '9999999')) ||
                   ' points de vie.<br>';
    if lanceur = cible then
...
ALTER FUNCTION public.nv_magie_mercurochrome(integer, integer, integer) OWNER TO delain;
COMMENT ON FUNCTION public.nv_magie_mercurochrome(integer, integer, integer) IS '   ';
