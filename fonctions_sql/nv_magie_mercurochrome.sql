CREATE OR REPLACE FUNCTION public.nv_magie_mercurochrome(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
	select into nom_cible,sexe_cible perso_nom,perso_sex from perso
...
-- on enlève les bonus existants
	select into v_pv,v_pv_max perso_pv,perso_pv_max from perso
		where perso_cod = cible;
	select into v_perso_niveau, v_perso_type perso_niveau, perso_type_perso from perso
...
-- on regarde si la personne est malade
        ...
        if temp_bonus < 0 then
	     ...
	     if v_bonus_flux != 0 then
              ...
              code_retour := code_retour||'<br>Vous gagnez '||bonus_flux||' points de vie grâce à la conscience des flux de soins.<br>';
              else
		...
		nouveau_pv := v_pv_max;
              ...
              update perso set perso_pv = nouveau_pv 
               where perso_cod = lanceur;
               bonus_flux := v_bonus_flux;
              code_retour := code_retour||'<br>Vous gagnez '||bonus_flux||' points de vie grâce à la conscience des flux de soins.<br>';
              end if;
...
-- on passe à l'augmentation de pvies de la cible
	...
	code_retour := code_retour||'<br>'||nom_cible||' a regagné '||trim(to_char(amel_pv,'9999999'))||' points de vie.<br>';
	if lanceur = cible then
