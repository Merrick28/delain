CREATE OR REPLACE FUNCTION public.mission_initialise_liberer(integer, integer)
 RETURNS boolean
 LANGUAGE plpgsql
AS $function$/*************************************************************/
/* fonction mission_initialise_liberer                       */
/*   Crée une mission de type « libérer une zone » dans      */
...
begin
	code_mission := 9;         -- Mission de type libérer une zone

	-- On détermine le rayon de la zone.
	v_rayon := 2 + lancer_des(1, 3);
	...
	v_recompense := v_difficulte * 200 + lancer_des(1, 200);

	-- On détermine le délai de réalisation (de 25 à 50 jours)
	v_delai := 24 + lancer_des(1, 26);
