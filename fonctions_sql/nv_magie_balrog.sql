CREATE OR REPLACE FUNCTION public.nv_magie_balrog(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
--------------------		   
	...
	code_retour := code_retour||'Vous provoquez '||trim(to_char(v_degats,'99999'))||' dégâts sur '||nom_cible||'<br>';
...
-- ensuite on fait les gens autour
	for ligne in select perso_cod,perso_nom,perso_pv
		...
		v_degats := lancer_des(1,10);
	-- on regarde si esquive
		code_retour := code_retour||'Sur <b>'||ligne.perso_nom||'</b>, vous provoquez '||trim(to_char(v_degats,'99999'))||' dégâts.<br>';
		texte_evt := texte_evt||' causant '||trim(to_char(v_degats,'999999'))||' dégâts';
