CREATE OR REPLACE FUNCTION public.ia_include_deplacement_vers(integer, integer, integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$/*****************************************************/
/* function ia_include_deplacement_vers :                        */
/*      Procédure utilisée par les différentes IA pour gérer les */
/*      déplacements vers une case donnée                        */
...
begin
	...
	while (v_pa > 0) loop
		...
		exit when compt_loop >= 6 OR temp_txt LIKE '1#Err%' OR temp_pos = pos_cible;
