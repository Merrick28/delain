CREATE or replace FUNCTION public.nv_magie_boule_feu_test(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
--------------------
    ...
    code_retour := code_retour || 'Vous provoquez ' || trim(to_char(v_degats, '99999')) || ' dégats sur ' ||
                   nom_cible || '<br>';
...
-- ensuite on fait les gens autour
    for ligne in select perso_cod, perso_nom, perso_pv, perso_pv_max
        ...
        loop
            ...
            select into v_res_feu bonus_valeur
            from bonus
            ...
            if not found then
                ...
                code_retour := code_retour || 'Degats d''origine:' || v_degats || ' avec ' || v_res_feu ||
                               ' % de résistance<br>';
                v_degats := max(0, (v_degats * (100 - v_res_feu)) / 100);
                v_res_feu := min(90, v_res_feu + lancer_des(1, 20));
                update bonus
                set bonus_valeur = v_res_feu
                where bonus_tbonus_libc = 'FEU'
                  and bonus_perso_cod = ligne.perso_cod;
            end if;
...
ALTER FUNCTION public.nv_magie_boule_feu_test(integer, integer, integer) OWNER TO delain;
