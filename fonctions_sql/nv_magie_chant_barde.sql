CREATE OR REPLACE FUNCTION public.nv_magie_chant_barde(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_chant_barde : lance le sort defense            */
...
-- on enlève les bonus existants
	...
	if v_bonus_toucher > 25 then
		v_bonus_toucher := 25;
	end if;
	
	perform ajoute_bonus(cible, 'PAA', 2, -1);
	perform ajoute_bonus(cible, 'TOU', 2, v_bonus_toucher);
	perform ajoute_bonus(cible, 'DEG', 2, 2);
	insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,5*ln(v_pv_cible));
	code_retour := code_retour||'<br>'||nom_cible||' gagne un bonus de -1PA par attaque, +2 de dégats et '||trim(to_char(v_bonus_toucher,'99999'))||' au toucher pendant 2 tours.<br>';
