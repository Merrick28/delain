CREATE or replace FUNCTION public.nv_magie_chant_barde(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- on enlève les bonus existants
    ...
    code_retour := code_retour || '<br>' || nom_cible || ' gagne un bonus de -1PA par attaque, +2 de dégats et ' ||
                   trim(to_char(v_bonus_toucher, '99999')) || ' au toucher pendant 2 tours.<br>';
...
ALTER FUNCTION public.nv_magie_chant_barde(integer, integer, integer) OWNER TO delain;
