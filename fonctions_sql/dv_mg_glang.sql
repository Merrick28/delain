CREATE OR REPLACE FUNCTION public.dv_mg_glang(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
	select into nom_cible,nv_pv_cible perso_nom,perso_pv_max from perso
	...
	select into nom_obj gobj_nom from objet_generique
	...
	v_degats := des;
	code_retour := code_retour||'Vous avez créé l''objet '||nom_obj||' au dessus de la cible. Celui-ci tombe en faisant '||trim(to_char(v_degats,'999999'))||' dégats.<br>';
	...
	select into v_pv_cible perso_pv from perso
		where perso_cod = cible;
	insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,0.5*ln(nv_pv_cible));
