...
CREATE FUNCTION public.f_repare_casque(integer, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/*******************************************************************/
...
begin
	...
	if v_des <= 5 then -- réussite critique
		...
		if (v_pa >= 2*getparm_n(40)) then
			code_retour := code_retour||'<a href="action.php?methode=repare&type=4&objet='||cast(num_objet as varchar(20))||'">Recommencer ('||cast(getparm_n(40) as varchar(2))||' PA)</a><br>';
		end if;
