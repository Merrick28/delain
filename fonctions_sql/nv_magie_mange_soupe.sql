CREATE or replace FUNCTION public.nv_magie_mange_soupe(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_mange_soupe : lance le sort Mange ta soupe     */
...
-------------------------------------------------------------
    ...
    v_perso_type             integer; -- type du lanceur (1=PJ, 2=monstre, 3=familier)
...
-------------------------------------------------------------	
    ...
    v_bonus_toucher := 5 * v_perso_niveau;
    -- On fixe à 25 pour les familiers
    if v_bonus_toucher > 25 or v_perso_type = 3 then
        v_bonus_toucher := 25;
    end if;
-- on enlève les bonus existants
    if ajoute_bonus(cible, 'TOU', 2, v_bonus_toucher) then

        insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee)
        values (3, lanceur, cible, 1.5 * ln(v_pv_cible));
    end if;
    code_retour := code_retour || '<br>' || nom_cible || ' bénéficie de ' || trim(to_char(v_bonus_toucher, '9999')) ||
                   ' points de bonus au toucher.;';
...
ALTER FUNCTION public.nv_magie_mange_soupe(integer, integer, integer) OWNER TO delain;
