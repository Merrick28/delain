CREATE OR REPLACE FUNCTION public.nv_magie_bipbip_groupe(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	select into v_perso_niveau,v_voie_magique,v_perso_int perso_niveau,perso_voie_magique,perso_int from perso
	...
	If v_voie_magique = 2 then
        	for ligne in
		select perso_cod,perso_nom from perso,perso_position
		where ppos_pos_cod = v_pos
               -- and perso_type_perso != '3'
		and ppos_perso_cod = perso_cod
		and perso_actif = 'O' loop

               	perform ajoute_bonus(ligne.perso_cod, 'DEP', 2, -2);
                code_retour := code_retour||'<br>'||ligne.perso_nom||' a un bonus de -2 en déplacement pendant 2 tours. Sans votre pouvoir de maître des arcanes votre influence aurait été moindre.';
	...
	--	and ppos_pos_cod = v_pos
	   ...
	   select perso_cod,perso_nom from perso,perso_position
		...
		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible].';
               	...
               	insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
    	 	values(nextval('seq_levt_cod'),14,now(),1,lanceur,texte_evt,'O','O',lanceur,ligne.perso_cod);   if (lanceur != cible) then
    insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
