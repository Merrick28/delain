CREATE OR REPLACE FUNCTION public.test_equipe(integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
/********************************************/
	select into pa,v_type_personnage perso_pa,perso_type_perso from perso where perso_cod = personnage;
	if not found then
...
/******************************************************/
/* Etape 4 : on vérifie que l objet est pas identifie */
/******************************************************/
...
/**************************************************************/
	select into compt count(obj_cod)
		...
		and perobj_equipe = 'O';
	code_retour := trim(to_char(compt,'999999'));
	return code_retour;

	if compt >= max_obj then
		code_retour := '1;Vous avez déjà '||trim(to_char(compt))||' objet(s) de ce type équipé ! (max '||trim(to_char(max_obj))||').';
		return code_retour;
	end if;

/****************************************/
/* Etape 8 : tout est vérifié, on passe */
/*   à la suite                         */
/****************************************/
-- 8.0 on enlève les transactions sur cet objet
...
-- 8.3 on met une ligne d evenement
	select into nom_perso perso_nom from perso
		where perso_cod = personnage;
	texte_evt := '[perso_cod1] a equipe l''objet n° '||trim(to_char(num_objet,'9999999999999'));
	insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible)
