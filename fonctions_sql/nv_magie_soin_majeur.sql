CREATE OR REPLACE FUNCTION public.nv_magie_soin_majeur(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- on passe à l'augmentation de pvies
	...
	if amel_pv > 0 then
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,3*ln(amel_pv));
	end if;
