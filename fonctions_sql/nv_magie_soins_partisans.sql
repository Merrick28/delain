CREATE or replace FUNCTION public.nv_magie_soins_partisans(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-------------------------------------------------------------	
    ...
    select into v_x,v_y,v_etage pos_x,
                                pos_y,
                                pos_etage
    from positions
    where pos_cod = v_pos;
    select into v_karma,v_perso_niveau,v_type_perso perso_kharma, perso_niveau, perso_type_perso
    from perso
    ...
    for ligne in select perso_cod, perso_nom, perso_pv, perso_pv_max, perso_sex
        ...
        loop
            ...
            -- et on les soigne
            select into v_pv,v_pv_max,nom_cible perso_pv, perso_pv_max, perso_nom
            from perso
            ...
            values (3, lanceur, ligne.perso_cod, (0.5 * ln(ligne.perso_pv_max) * amel_pv) / nb_cible);
...
ALTER FUNCTION public.nv_magie_soins_partisans(integer, integer, integer) OWNER TO delain;
