CREATE OR REPLACE FUNCTION public.f_passe_niveau(integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/********************************************************/
...
perform guard('f_passe_niveau'); -- Ne pas oublier le perform release à chaque branche de sortie de fonction !
-- etape 1 : on vérifie
    select into     v_pa,
                        v_niveau_actu,
                        v_limite_niveau,
    ...
    if not found then
        ...
        perform release('f_passe_niveau');
        return code_retour;
    end if;
