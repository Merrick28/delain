CREATE or replace FUNCTION public.dv_mg_golem_arme(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_mg_golem_arme : lance le sort golem d'arme        */
...
-- OK on créé le golem de brouzoufs	
    v_monstre := cree_monstre_pos(535, cible);
    code_retour := code_retour || '<br>Vous avez invoqué avec succès un golem d''arme et d''armure.';
...
ALTER FUNCTION public.dv_mg_golem_arme(integer, integer, integer) OWNER TO delain;
