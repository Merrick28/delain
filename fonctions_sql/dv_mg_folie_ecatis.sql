CREATE or replace FUNCTION public.dv_mg_folie_ecatis(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_mg_folie_ecatis                                   */
...
-------------------------------------------------------------	
    ...
    perform ajoute_bonus(cible, 'FEC', 2, v_niveau_dper * 15);
    code_retour := code_retour || 'La cible subit la folie d''Ecatis.<br>';
...
ALTER FUNCTION public.dv_mg_folie_ecatis(integer, integer, integer) OWNER TO delain;
