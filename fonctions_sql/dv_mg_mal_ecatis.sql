CREATE OR REPLACE FUNCTION public.dv_mg_mal_ecatis(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function dv_mg_mal_ecatis lance le sort malediction d'ecatis  */
...
-------------------------------------------------------------	
	...
	perform ajoute_bonus(cible, 'MEC', 3, 10);
	code_retour := code_retour||'La cible est maudite par Ecatis, les dégats qui lui sont fait par des nains, elfes, humains et leurs familiers  sont accrus.<br>';
	...
	insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,0.5*ln(v_pv_cible_m));
