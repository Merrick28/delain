CREATE or replace FUNCTION public.nv_magie_botte_secrete(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_defense : lance le sort defense                */
...
-------------------------------------------------------------	
    ...
    if ajoute_bonus(cible, 'PAA', 2, -1) then

        insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee)
        values (3, lanceur, cible, 1.5 * ln(v_pv_cible));
    end if;
    code_retour := code_retour || '<br>' || nom_cible || ' gagne un bonus de -1PA par attaque pendant 2 tours.<br>';
...
ALTER FUNCTION public.nv_magie_botte_secrete(integer, integer, integer) OWNER TO delain;
