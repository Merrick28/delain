CREATE OR REPLACE FUNCTION public.nv_magie_poisse(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
begin
	...
	code_retour := code_retour||'<br>'||nom_cible||' a un malus au toucher de '||trim(to_char(v_bonus_toucher,'99'))||'% pendant 2 tours.';
