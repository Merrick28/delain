CREATE or replace FUNCTION public.dv_mg_farfadet(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
-- niveau du lanceur
    ...
    v_bonus := lancer_des(50, 20);
    insert into or_position
        (por_pos_cod, por_qte, por_palpable)
    values (cible, v_bonus, 'N');

    code_retour := code_retour || '<br>' || 'Vous avez créé ' || trim(to_char(v_bonus, '999999')) ||
                   ' brouzoufs de farfadets sur la case.<br>';
...
ALTER FUNCTION public.dv_mg_farfadet(integer, integer, integer) OWNER TO delain;
