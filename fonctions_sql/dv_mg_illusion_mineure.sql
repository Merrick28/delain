CREATE or replace FUNCTION public.dv_mg_illusion_mineure(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
...
/*   18/04/2006 : Correction de la sortie évènement              */
/*****************************************************************/
declare
-------------------------------------------------------------
...
-------------------------------------------------------------	
    ...
    if des_objet < 20 then
        v_monstre := 1;
    end if;
    if des_objet >= 20 and des_objet < 40 then
        v_monstre := 2;
    end if;
    if des_objet >= 40 and des_objet < 60 then
        v_monstre := 3;
    end if;
    if des_objet >= 60 and des_objet < 80 then
        v_monstre := 4;
    end if;
    if des_objet >= 80 then
        v_monstre := 10;
    end if;
    select into nom_monstre gmon_nom
    from monstre_generique
    where gmon_cod = v_monstre;

    v_monstre := cree_monstre_pos(v_monstre, v_pos_cible);
    ...
    code_retour := code_retour || 'Vous avez invoqué ' || nom_monstre || ' qui attaque votre cible.<br>';
...
ALTER FUNCTION public.dv_mg_illusion_mineure(integer, integer, integer) OWNER TO delain;
