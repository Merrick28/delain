CREATE OR REPLACE FUNCTION public.nv_magie_transfert_pouvoir(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- magie non résistée --
------------------------
	code_retour := code_retour||'Votre adversaire n''arrive pas à résister au sort, vous lui volez tous ses bonus.<br>';
-- effet de la voie magique
        ...
        code_retour := code_retour||' En tant que sorcier vous tuez lentement votre cible, infectant le sang de cette dernière pour 3 tours.<br>';
        end if;
