CREATE or replace FUNCTION public.dv_mg_petit_marteau_ecatis(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_mg_petit_marteau_ecatis                           */
    ...
    where obj_cod in
          (select perobj_obj_cod
           ...
           where perobj_perso_cod = lanceur
             ...
             and gobj_tobj_cod in (1, 2, 4, 9, 10));
    code_retour := code_retour || 'Tous vos objets équipés ont été remis à neuf, à l''exception des artefacts.<br>';
...
ALTER FUNCTION public.dv_mg_petit_marteau_ecatis(integer, integer, integer) OWNER TO delain;
