CREATE OR REPLACE FUNCTION public.nv_magie_toucher_glacial(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	select into v_perso_niveau,v_int_lanceur,v_voie_magique perso_niveau,perso_int,perso_voie_magique from perso
	...
	if v_voie_magique = 4 then
		...
		code_retour := code_retour||'Votre puissance de mage de guerre se fait sentir au travers de ce sortilège.<br>';
	else
		bonus_voie := 0;
	end if;
...
------------------------
			...
			if ajoute_bonus(cible, 'PAA', 2, 2) != 0 then
				insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,0.5*ln(v_pv_cible));
			end if;
			code_retour := code_retour||'<br>'||nom_cible||' se retrouve totalement engourdie et subit un malus de 2 PA pour ses attaques.<br>';
		else
--------------------
-- magie résistée --
--------------------		   
      ...
      degats := degats + (v_int_lanceur) + bonus_voie;
		...
		-- on enlève les bonus existants
			if ajoute_bonus(cible, 'PAA', 2, 1) != 0 then
				insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,1*ln(v_pv_cible));
			end if;
			code_retour := code_retour||'<br>'||nom_cible||' se retrouve partiellement engourdie et subit un malus de 1 PA pour ses attaques.<br>';
		end if;	
		...
		degats := des;
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (2,lanceur,cible,0.25*ln(v_pv_cible)*degats);
    code_retour := code_retour||'<br>'||nom_cible||' perd '||trim(to_char(degats,'999'))||' points de vie.<br>';
		...
		-- pas tuée	
			...
			update perso set perso_pv = perso_pv - degats
				where perso_cod = cible;
		else
