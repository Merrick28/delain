CREATE or replace FUNCTION public.dv_mg_faiblesse(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_mg_faiblesse : lance le sort faiblesse            */
...
-------------------------------------------------------------	
    ...
    perform ajoute_bonus(cible, 'DEG', 2, -v_niveau_dper);
    code_retour := code_retour || 'La cible subit ' || trim(to_char(v_niveau_dper, '999999')) ||
                   ' de malus aux dégats.<br>';
...
ALTER FUNCTION public.dv_mg_faiblesse(integer, integer, integer) OWNER TO delain;
