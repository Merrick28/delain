CREATE or replace FUNCTION public.nv_magie_botte_magique(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_botte_magique : lance le sort botte magique    */
...
-- on enlève les bonus existants
    if ajoute_bonus(cible, 'PAM', 2, -2) != 0 then

        insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee)
        values (3, lanceur, cible, 0.5 * ln(v_pv_cible));
    end if;
    code_retour := code_retour || '<br>' || nom_cible || ' gagne un bonus de -2PA par sort pendant 2 tours.<br>';
...
ALTER FUNCTION public.nv_magie_botte_magique(integer, integer, integer) OWNER TO delain;
