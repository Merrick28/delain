CREATE FUNCTION public.dv_mg_chance(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_mg_chance : lance le sort chance                  */
...
-- niveau du lanceur
    select into v_niveau_lanceur dper_niveau
    from dieu_perso
    where dper_perso_cod = lanceur;
    v_bonus := v_niveau_lanceur * 15;
-- on enlève les bonus existants
    perform ajoute_bonus(cible, 'ESQ', 2, v_bonus);
    insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (3, lanceur, cible, ln(v_pv_cible));
    code_retour := code_retour || '<br>' || nom_cible || ' a un bonus de ' || trim(to_char(v_bonus, '999999')) ||
                   ' en esquive pendant 2 tours.<br>';
...
ALTER FUNCTION public.dv_mg_chance(integer, integer, integer) OWNER TO delain;
