CREATE or replace FUNCTION public.nv_magie_glu(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_glu : lance le sort                            */
...
-------------------------------------------------------------
    ...
    nb_monstre               integer;
    num_monstre              integer;
    v_perso_type             integer; --type de perso
    v_gmon_cod               integer;
-------------------------------------------------------------
-- variables de calcul
-------------------------------------------------------------
...
/*Différenciation entre l'invocation de gelées bleues invoquées par les monstres et celles invoquées par les joueurs*/


    select into v_perso_type perso_type_perso from perso where perso_cod = lanceur;

    if v_perso_type = 2 then
        v_gmon_cod = 46;
    else
        v_gmon_cod = 862;
    end if;
...
-- OK on créé la gelée bleue	
    v_monstre := cree_monstre_pos(v_gmon_cod, v_pos);
    code_retour := code_retour || '<br>Vous avez invoqué avec succès une gelée bleue.';
...
ALTER FUNCTION public.nv_magie_glu(integer, integer, integer) OWNER TO delain;
