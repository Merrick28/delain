CREATE OR REPLACE FUNCTION public.nv_magie_boule_feu_test4(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- ensuite on fait les gens autour
	for ligne in select perso_cod,perso_nom,perso_pv,perso_pv_max
		...
		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible]';
		v_degats := lancer_des(1,10);
                code_retour := code_retour||'(deg = deg -1D100%armure )DEGATS D''ORIGINE:'||v_degats||'<br>';
