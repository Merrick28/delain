CREATE OR REPLACE FUNCTION public.nv_magie_bipbip_groupe2(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	select into v_perso_niveau,v_voie_magique,v_perso_int,lanceur_nom,pos_lanceur perso_niveau,perso_voie_magique,perso_int,perso_nom,ppos_pos_cod from perso,perso_position
...
/*        v_perso_niveau := floor(v_perso_niveau/10);
	...
	If v_voie_magique = 2 then
		...
		texte_add := '';
        	...
        	for ligne in
							select perso_cod,perso_nom from perso,perso_position /*On détermine les cibles potentielles, en excluant le lanceur et les familiers*/
                ...
                code_retour := code_retour||'<br>'||ligne.perso_nom||' a un bonus de -2 en déplacement pendant '||cast(duree_cible as text)||' tours. '||texte_add;
...
if (lanceur != ligne.perso_cod) then
    insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
      ...
      -- on traite ensuite le maitres des arcanes en particulier
	      ...
	      code_retour := code_retour||'<br>'||lanceur_nom||' a un bonus de -2 en déplacement pendant '||cast(duree_lanceur as text)||' tours. '||texte_add;
	      texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible].';
	      insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
