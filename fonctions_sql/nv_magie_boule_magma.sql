CREATE OR REPLACE FUNCTION public.nv_magie_boule_magma(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
v_degats := des;
	...
	code_retour := code_retour||'Vous provoquez '||trim(to_char(v_degats,'99999'))||' dégats sur <b>'||nom_cible||'</b><br>';
        code_retour := code_retour||'La boule de magma inflige ' ||text_brulure||' de brulûres profondes durant deux tours.<br>';
