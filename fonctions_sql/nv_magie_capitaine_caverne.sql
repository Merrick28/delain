CREATE OR REPLACE FUNCTION public.nv_magie_capitaine_caverne(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------
	...
	-- Création de l’objet
	SELECT INTO code_objet, nom_objet, poids_objet gobj_cod, gobj_nom, gobj_poids FROM objet_generique
	WHERE 
		(v_bloque_magie = 0 AND gobj_cod IN (60, 61, 62, 63, 64, 65, 66, 163, 256, 560)) OR           -- Magie non résistée
	...
	if v_bloque_magie = 0 then -- magie non résistée --
		...
		insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (2, lanceur, cible, 1.5 * ln(v_pv_cible));
		texte_evt := texte_evt || ', qui a partiellement résisté au sort.';
	end if;

	code_retour := code_retour || '<br>Vous avez créé l’objet <b>' || nom_objet || '</b>, pesant ' || poids_objet::text || ', dans l’inventaire de ' || nom_cible || '.<br>';
