CREATE OR REPLACE FUNCTION public.dv_mg_aimant(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	select into v_x,v_y,v_e 
		pos_x,pos_y,pos_etage from positions,perso_position
		where ppos_perso_cod = lanceur
		and ppos_pos_cod = pos_cod;
	cpt_brouzoufs := 0;
	for ligne in select pos_cod from positions
		where pos_etage = v_e
		and pos_x between (v_x - 1) and (v_x + 1)
		and pos_y between (v_y - 1) and (v_y + 1) loop
		select into tmp_brouzoufs sum(por_qte)
			from or_position
			where por_pos_cod = ligne.pos_cod and por_palpable = 'O';
	...
	code_retour := code_retour||'Vous avez récupéré '||trim(to_char(cpt_brouzoufs,'999999999999999999'))||' dans votre inventaire.<br>';
