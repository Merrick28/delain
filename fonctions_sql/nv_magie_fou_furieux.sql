CREATE OR REPLACE FUNCTION public.nv_magie_fou_furieux(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_defense : lance le sort fou furieux            */
...
-- on enlève les bonus existants
	perform ajoute_bonus(cible, 'PAA', 1, -3);
	code_retour := code_retour||'<br>'||nom_cible||' gagne un bonus de -3PA par attaque pendant 1 tour (minimum 2PA par attaque).<br>';
