CREATE OR REPLACE FUNCTION public.ia_include_deplacement_aleatoire(integer, integer, integer)
 RETURNS integer
 LANGUAGE plpgsql
AS $function$/*****************************************************/
/* function ia_include_deplacement_aleatoire :                   */
...
declare
	...
	v_vue alias for $3;        -- la limite de vue du monstre
	pos_cible integer;         -- la position ciblée
...
begin
	...
	while (v_pa > 0) loop
		...
		exit when compt_loop >= 6 OR temp_txt LIKE '1#Err%';
