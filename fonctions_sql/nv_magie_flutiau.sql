CREATE or replace FUNCTION public.nv_magie_flutiau(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_flutiau : lance le sort                        */
...
-- OK on créé l'élémentaire	
    nb_monstre := lancer_des(1, 10);
    for num_monstre in 1..nb_monstre
        loop
            v_monstre := cree_monstre_pos(2, v_pos);
        end loop;
...
ALTER FUNCTION public.nv_magie_flutiau(integer, integer, integer) OWNER TO delain;
