--
-- Name: pot_remede(integer); Type: FUNCTION; Schema: potions; Owner: postgres
--

CREATE or replace FUNCTION potions.pot_remede(integer) RETURNS text
LANGUAGE plpgsql
AS $_$/*********************************************************/
/* function pot_remede						                           */
/* parametres :                                          */
...
begin
  ...
  delete from bonus where
    ...
    code_retour := code_retour||'Vous avez supprimé tous les bonus/malus liés à la maladie du sang';
  end if;
  return code_retour;
end;
$_$;
...
ALTER FUNCTION potions.pot_remede(integer) OWNER TO delain;