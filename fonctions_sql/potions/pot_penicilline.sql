--
-- Name: pot_penicilline(integer,integer); Type: FUNCTION; Schema: potions; Owner: postgres
--

CREATE or replace FUNCTION potions.pot_penicilline(integer,integer) RETURNS text
LANGUAGE plpgsql
AS $_$/*********************************************************/
/* function pot_penicilline						                           */
/* parametres :                                          */
  ...
  /*Tous les controles sont OK, on passe alors aux effets de la potion uniquement*/
	...
	delete from bonus where
	  ...
	  bonus_mode != 'E' and   -- sauf bonus d'équipement
		bonus_tbonus_libc = 'MAL';

	if cible = personnage then
		code_retour := code_retour || 'Vous avez supprimé tous vos bonus/malus liés à la maladie';
	else
		code_retour := code_retour || 'Vous avez supprimé tous les bonus/malus du buveur liés à la maladie';
	end if;
  end if;
  return code_retour;
end;
...
ALTER FUNCTION potions.pot_penicilline(integer,integer) OWNER TO delain;