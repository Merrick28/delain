--
-- Name: pot_serum(integer); Type: FUNCTION; Schema: potions; Owner: postgres
--

CREATE or replace FUNCTION potions.pot_serum(integer) RETURNS text
LANGUAGE plpgsql
AS $_$/*********************************************************/
/* function pot_serum						                           */
/* parametres :                                          */
...
begin
  ...
  delete from bonus where
    ...
    code_retour := code_retour||'Vous avez supprimé tous les bonus/malus liés au venin';
  end if;
  return code_retour;
end;
$_$;
...
ALTER FUNCTION potions.pot_serum(integer) OWNER TO delain;