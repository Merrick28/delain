--
-- Name: potion_toxique(integer); Type: FUNCTION; Schema: potions; Owner: delain
--

CREATE or replace FUNCTION potions.potion_toxique(integer) RETURNS text
LANGUAGE plpgsql
AS $_$/****************************************************/
/* potion_toxique                                   */
...
begin
  ...
  if v_des <= 10 then
    ...
    perform public.ajoute_bonus(personnage, '0RG', 2, 1);
    ...
    perform public.ajoute_bonus(personnage, 'REG', v_temps_perte_regen, -v_perte_regen);
    ...
    where perso_cod = personnage;
    v_temps_perte_regen := floor(v_constit/4);
    perform public.ajoute_bonus(personnage, 'VUE', v_temps_perte_regen, -10);
...
ALTER FUNCTION potions.potion_toxique(integer) OWNER TO delain;