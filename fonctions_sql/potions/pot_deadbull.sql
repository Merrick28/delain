--
-- Name: pot_deadbull(integer); Type: FUNCTION; Schema: potions; Owner: postgres
--

CREATE or replace FUNCTION potions.pot_deadbull(integer) RETURNS text
LANGUAGE plpgsql
AS $_$/*********************************************************/
...
begin
  ...
  delete from bonus where
    bonus_perso_cod = personnage and
    bonus_tbonus_libc = 'DIS';


    code_retour := code_retour||'Vous avez supprimé tous les bonus/malus liés à la lenteur temporelle';
  end if;
  return code_retour;
end;
$_$;
...
ALTER FUNCTION potions.pot_deadbull(integer) OWNER TO delain;