CREATE OR REPLACE FUNCTION public.nv_magie_armure_symbiotique(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- on met à jour les variables de travail
	select into v_pv,v_pv_max perso_pv,perso_pv_max from perso
		where perso_cod = cible;
	select into v_perso_int,v_perso_for,v_perso_dex,v_perso_con,v_voie_magique perso_int,perso_force,perso_dexterité,perso_constitution,perso_voie_magique from perso
       ...
       -- augmentation due à la voie magique
              ...
              v_compt := v_compt + 1;
              code_retour := code_retour||'<br>Votre savoir de mage de guerre vous permet de prolonger plus longtemps l''armure symbiotique.<br>'; 
              end if;    
	...
	if ajoute_bonus(cible, 'GLA', v_compt, v_puissance) != 0 then
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee) values (3,lanceur,cible,0.1*ln(v_pv_cible));
	end if;
	...
	code_retour := code_retour||'<br>'||nom_cible||' devient entouree d''une armure symbiotique pour'|| v_compt||' tours.<br>';
