CREATE OR REPLACE FUNCTION public.nv_magie_conjuration_majeure(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-------------------------------------------------------------	
	...
	if niveau_lanceur > niveau_cible then
		code_retour := code_retour||'Ce sort ne peut impacter que des monstres plus puissants que vous.<br />Votre adversaire n’en ressent aucun effet.<br>';
	else
