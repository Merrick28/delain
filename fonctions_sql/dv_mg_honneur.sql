CREATE or replace FUNCTION public.dv_mg_honneur(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function dv_honneur : lance le sort                           */
...
-------------------------------------------------------------
    ...
    v_pv_cible               integer; -- PV de la cible
-------------------------------------------------------------
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
    select into nom_cible,v_pv_cible perso_nom, perso_pv
    from perso
    ...
    perform ajoute_bonus(cible, 'HON', 2, 20);
    insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee)
    values (3, lanceur, cible, 3 * ln(v_pv_cible));
    code_retour := code_retour || '<b>' || nom_cible || '</b> est maintenant sous l''effet du sort honneur.<br>';
...
ALTER FUNCTION public.dv_mg_honneur(integer, integer, integer) OWNER TO delain;
