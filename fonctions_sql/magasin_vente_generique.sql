--
-- Name: magasin_vente_generique(integer, integer, integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION magasin_vente_generique(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/**************************************************************************/
...
----------------------------------------------------------------------------
  ...
  insert into mag_tran_generique (mgtra_lieu_cod,mgtra_perso_cod,mgtra_gobj_cod,mgtra_sens,mgtra_montant,mgtra_nombre)
		values
		(magasin,personnage,v_obj_type,0,prix_objet,1);
...
-- on remet l'objet à neuf
    ...
    if not found then
		...
		if type_magasin = 14 then
		  -- historiquement les magasins runiques étaient auto-gérés tous les objets vendus, pouvaient être acheté par d'autre.
		  update stock_magasin_generique
        set mgstock_vente_persos = 'O'
...
ALTER FUNCTION public.magasin_vente_generique(integer, integer, integer) OWNER TO delain;
