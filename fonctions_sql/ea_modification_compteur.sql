--
-- Name: ea_modification_compteur(integer, integer, integer, character, text, numeric, text, json); Type: FUNCTION; Schema: public; Owner: delain
--

create or replace function ea_modification_compteur(integer, integer, integer, character, text, numeric, text, json) RETURNS text
LANGUAGE plpgsql
AS $_$/**************************************************/
/* ea_modification_compteur                             */
...
declare
  ...
  ligne record;                -- Une ligne d’enregistrements sur les persos
  cpt record;                  -- Une ligne d’enregistrements sur les compteurs
  ...
  v_compteur_cod  integer;        -- code du compteur à modifier
  v_sens integer;              -- sens de modification du compteur
begin
  ...
  loop
    ...
    loop
        ...
        if (v_compteur_cod > 0) and (v_sens>=-1) and (v_sens<=1) then

            -- faire la modification du compteur
            perform f_compteur_modif(v_compteur_cod, ligne.perso_cod, cpt.value->>'valeur', v_sens);

            -- On rajoute la ligne d’événements pour l'utilisateur
            if v_texte_evt != '' then
        ...
        end if; -- mauvais paramètre d'entrée

    end loop;   -- end loop sur les compteurs

  end loop; -- end loop sur les cibles
...
ALTER FUNCTION public.ea_modification_compteur(integer, integer, integer, character, text, numeric, text, json) OWNER TO delain;
