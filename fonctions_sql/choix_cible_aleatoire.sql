...
/* ou un monstre, choisir une cible dans sa vue           */
...
begin
  ...
  if found then
    if v_type_perso = 2 then
      ...
      -- cas d'un monstre qui cherche une cible à porté de frappe
      ...
      -- cas d'un perso qui cherche une cible à porté de frappe (un perso qui n'est pas sur le même compte)
      select into code_retour perso_cod
        from perso,positions,perso_position
        where pos_x between (v_pos_x-v_portee) and (v_pos_x+v_portee)
        and pos_y between (v_pos_y-v_portee) and (v_pos_y+v_portee)
...
ALTER FUNCTION public.choix_cible_aleatoire(integer, integer)
  OWNER TO delain;
GRANT EXECUTE ON FUNCTION public.choix_cible_aleatoire(integer, integer) TO delain;
