...
--
-- Name: enlumineur(integer, integer); Type: FUNCTION; Schema: public; Owner: delain
--

CREATE OR REPLACE FUNCTION public.enlumineur(integer, integer) RETURNS text
    LANGUAGE plpgsql
    AS $_$/************************************************/
/* enlumineur				        			              */
...
--Fin de la quête premier niveau
	...
	--On supprime 1 objet de quête type plume de scribe
	delete from perso_objets where perobj_perso_cod = personnage and perobj_obj_cod in (select obj_cod from perso_objets join objets on obj_cod=perobj_obj_cod where perobj_perso_cod=personnage and obj_gobj_cod =730 limit 1);
	--On supprime 1 objet de quête type peau de demon travaillé
	delete from perso_objets where perobj_perso_cod = personnage and perobj_obj_cod in (select obj_cod from perso_objets join objets on obj_cod=perobj_obj_cod where perobj_perso_cod=personnage and obj_gobj_cod =731 limit 1);
...
ALTER FUNCTION public.enlumineur(integer, integer) OWNER TO delain;
