CREATE or replace FUNCTION public.nv_magie_lance_eclair(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
/* function magie_lance_eclair : lance le sort épée de feu           */
...
-- on vérifié qu'il n'y ait pas une arme
    ...
    insert into objets (obj_cod, obj_gobj_cod, obj_portee) values (num_objet, 606, 1);
    ...
    code_retour := code_retour ||
                   '<br>Vous avez dans votre main une lance d''éclair. Celle ci existera pendant 48 heures.';
...
ALTER FUNCTION public.nv_magie_lance_eclair(integer, integer, integer) OWNER TO delain;
