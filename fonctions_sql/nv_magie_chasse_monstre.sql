CREATE OR REPLACE FUNCTION public.nv_magie_chasse_monstre(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
/* function magie_chasse_monstre : lance le sort chasse monstre  */
...
-------------------------------------------------------------
	...
	nom_cible text;				-- nom de la cible
        v_niv_degats integer;                   -- multiplicateur de dégats par niveau de cible
-------------------------------------------------------------
...
-- on commence par les gens sur la même case
	...
	for ligne in select perso_cod,perso_nom,perso_pv,perso_pv_max,perso_niveau
		...
		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible]';
                v_niv_degats := floor(ligne.perso_niveau*0.75);
		v_degats := lancer_des(1,v_niv_degats);
                -- on regarde si esquive
		code_retour := code_retour||'Sur <b>'||ligne.perso_nom||'</b>, vous provoquez '||trim(to_char(v_degats,'99999'))||' dégats';
		...
		if reussite_esquive != 0 then
   		...
   		insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
			...
			code_retour := code_retour||' que votre adversaire n''arrive pas à esquiver.<br>';
                        -- esquive ratée, pour le mage de guerre on supprime le lock de combat de la cible
                                if v_voie_magique = '4'then
                                   delete from lock_combat where lock_cible = lanceur
                                                            and lock_attaquant = ligne.perso_cod;
                                   delete from lock_combat where lock_cible = ligne.perso_cod
                                                            and lock_attaquant = lanceur;
                               code_retour := code_retour||'Votre pouvoir de mage de guerre vous a permis de rompre avec '||ligne.perso_nom||' votre lock de combat. ';
                                end if;
			insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
     			values(nextval('seq_levt_cod'),14,now(),1,lanceur,texte_evt,'O','O',lanceur,ligne.perso_cod);
   		insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
...
-- ensuite on fait les gens à 1 de distance
	for ligne in select perso_cod,perso_nom,perso_pv,perso_pv_max,perso_niveau
		...
		where lpos_pos_cod = pos_cod
		and lieu_refuge = 'O')
		and pos_etage = e_lanceur loop
		texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible]';
		v_degats := lancer_des(1,ligne.perso_niveau);
                v_degats := floor(v_degats/3);
	-- on regarde si esquive
		code_retour := code_retour||'Sur <b>'||ligne.perso_nom||'</b>, vous provoquez '||trim(to_char(v_degats,'99999'))||' dégats';
		insert into action (act_tact_cod,act_perso1,act_perso2,act_donnee)
...
if (lanceur != ligne.perso_cod) then
    insert into ligne_evt(levt_cod,levt_tevt_cod,levt_date,levt_type_per1,levt_perso_cod1,levt_texte,levt_lu,levt_visible,levt_attaquant,levt_cible)
     	values(nextval('seq_levt_cod'),14,now(),1,cible,texte_evt,'N','O',lanceur,ligne.perso_cod);
   end if;
