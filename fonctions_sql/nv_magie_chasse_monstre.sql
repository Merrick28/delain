CREATE or replace FUNCTION public.nv_magie_chasse_monstre(integer, integer, integer) RETURNS text
    LANGUAGE plpgsql
AS
$_$/*****************************************************************/
        ...
        loop
            ...
            if reussite_esquive != 0 then
                ...
                if v_voie_magique = '4' then
                    delete
                    from lock_combat
                    where lock_cible = lanceur
                      and lock_attaquant = ligne.perso_cod;
                    delete
                    from lock_combat
                    where lock_cible = ligne.perso_cod
                      and lock_attaquant = lanceur;
                    code_retour := code_retour || 'Votre pouvoir de mage de guerre vous a permis de rompre avec ' ||
                                   ligne.perso_nom || ' votre lock de combat. ';
                end if;
                insert into ligne_evt(levt_cod, levt_tevt_cod, levt_date, levt_type_per1, levt_perso_cod1, levt_texte,
                                      levt_lu, levt_visible, levt_attaquant, levt_cible)
                values (nextval('seq_levt_cod'), 14, now(), 1, lanceur, texte_evt, 'O', 'O', lanceur, ligne.perso_cod);
        ...
        loop
            ...
            if reussite_esquive != 0 then
                ...
                if (lanceur != ligne.perso_cod) then
                    ...
                    values (nextval('seq_levt_cod'), 14, now(), 1, cible, texte_evt, 'N', 'O', lanceur,
                            ligne.perso_cod);
                end if;
...
ALTER FUNCTION public.nv_magie_chasse_monstre(integer, integer, integer) OWNER TO delain;
