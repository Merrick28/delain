CREATE OR REPLACE FUNCTION public.nv_magie_berseker(integer, integer, integer)
 RETURNS text
 LANGUAGE plpgsql
AS $function$/*****************************************************************/
...
-- Etape 2 : contrôles
-------------------------------------------------------------	
	select into nom_cible,v_pv_cible
		perso_nom,perso_pv_max
	from perso
...
-- sélection sur le lanceur
	select into v_perso_niveau, v_int_perso, v_voie_magique
		perso_niveau, perso_int, perso_voie_magique
	from perso
	...
	v_duree_txt := v_duree;
	code_retour := code_retour||'<br>'||nom_cible||' gagne un bonus de -2PA par attaque et +4 de dégâts pendant ' ||v_duree_txt|| ' tours.<br>';
	...
	code_retour := code_retour||'<br>'||nom_cible||' perd 150% en esquive durant ' ||v_duree_txt|| ' tours.<br>';
	code_retour := code_retour||'<br>Vous gagnez '||px_gagne||' PX pour cette action.<br>';

	texte_evt := '[attaquant] a lancé '||nom_sort||' sur [cible].';
	insert into action (act_tact_cod, act_perso1, act_perso2, act_donnee) values (3, lanceur, cible, 3.5*ln(v_pv_cible));
